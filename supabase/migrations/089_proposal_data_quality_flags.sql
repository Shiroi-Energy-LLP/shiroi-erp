-- ============================================================
-- Migration 089 — Proposal data quality flags
-- File: supabase/migrations/089_proposal_data_quality_flags.sql
...
COMMENT ON COLUMN proposals.financials_invalidated IS
  'Set to TRUE when migration 090 reset the financial fields after detecting corruption (per-kWp > ₹2L confident or > ₹5L doubtful). Cleared when a fresh quote/import populates valid numbers.';

COMMENT ON COLUMN proposals.system_size_uncertain IS
  'Set to TRUE when system_size_kwp could not be corroborated against lead.estimated_size_kwp or projects.system_size_kwp at the time of migration 090. The kWp itself needs verification, not just the price.';
