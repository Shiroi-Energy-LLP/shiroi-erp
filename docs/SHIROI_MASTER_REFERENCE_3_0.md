...
| Price Book CSV import | ✅ Complete | `scripts/import-price-book-csv.ts` — minimal CSV parser, 22-category CATEGORY_MAP, rate parser strips ₹+commas, batched 100/row inserts via service key, --dry-run flag. **Blocker:** Google Sheet is private, requires founder to export locally first. Ready to run: `npx tsx scripts/import-price-book-csv.ts path/to/export.csv`. |
