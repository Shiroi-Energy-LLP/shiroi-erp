...
**Version 3.18 | Updated April 15, 2026 | Read before every coding session**
...
| Engineering rules codified | ✅ Complete (Apr 14) | 10 NEVER-DO rules (CLAUDE.md §11–20) + new §4.11 in this doc. Cover `as any`/`: any` in Supabase queries, JS money aggregation, `count: 'exact'` on large tables, form >500 LOC, inline Supabase in pages/components, time-series in regular tables, missing indexes, background work in server actions, throws from actions, schema changes without type regen. All 10 rules emerged from April 14 full-codebase audit (3 parallel Explore agents). Plus CODING STANDARDS subsections for row types, `ActionResult<T>`, query/action file separation, SQL aggregation via RPCs, declarative partitioning template, "when in doubt add an index". |
| GitHub Actions CI | ✅ Complete (Apr 14) | `.github/workflows/ci.yml`: pnpm check-types (4 packages) + pnpm lint (2 packages @ `--max-warnings 0`) + `scripts/ci/check-forbidden-patterns.sh` baseline-aware grep for NEVER-DO rules 11/13/15. Runs on PR + push to main. Typical ~1m. Pre-work: migrated apps/erp from `next lint` → eslint flat config, added Node globals for .config.* files, 16 env vars to turbo.json globalEnv, eslint-config downgrades (no-explicit-any/no-unused-vars/no-img-element/exhaustive-deps → off with TODO cleanup targets), react/prop-types off in shared config. **Fixed 4 real react-hooks/rules-of-hooks bugs** in lead-files.tsx (early return was before hooks — would crash when files flipped empty/non-empty). |
| Forbidden-pattern ratchet | ✅ Complete (Apr 14) | Baseline file `scripts/ci/.forbidden-patterns-baseline` grandfathers existing violations; new ones block CI. Baseline ratchets DOWN only. Started at 99, dropped to 97 after migration 048 removed 2 `count: 'exact'` call sites. |
| ActionResult<T> helper | ✅ Complete (Apr 14) | `apps/erp/src/lib/types/actions.ts` — canonical `ActionResult<T>` discriminated union + `ok`/`err`/`isOk` helpers. Template for upcoming refactor of all 56 `*-actions.ts` files per NEVER-DO rule #19 (no throws, return typed results). |
| Migration 048 | ✅ Applied (dev, Apr 14) | Performance round 2: 4 indexes + 3 aggregation RPCs. **Indexes**: `activity_associations(entity_id, entity_type)`, `proposal_bom_lines(proposal_id, gst_type)`, `customer_payments(project_id, payment_date DESC)`, `daily_site_reports(project_id, report_date DESC)`. **RPCs**: `get_pipeline_summary()` (replaces JS reduce over draft/sent/negotiating proposals — at 10x scale was pushing ~375k rows/min through JS heap for 50-user founder dashboard; dev: 413 proposals, ₹1.37T), `get_projects_without_today_report()` (IST-aware NOT EXISTS anti-join replaces 2-query N+1 + client filter; dev: 16 projects), `get_amc_monthly_summary()` (single FILTER-clause query replaces 2× `count: 'exact'` head queries; dev: 1 scheduled, 1 completed). All 3 RPCs wired in `apps/erp/src/lib/dashboard-queries.ts` deleting JS reduces. Types regenerated. Prod pending. |
| As-any cleanup R1 | ✅ Complete (Apr 15) | 5 files refactored to typed rows + `ActionResult<T>`: `amc-actions.ts` (16 `from('X' as any)` violations → 0), `data-flag-actions.ts` (8→0), `service-ticket-actions.ts` (6→0, `TicketStatus` enum now exported), `tasks-actions.ts` (6→0), `ticket-queries.ts` (2→0). All callers untouched — `ActionResult<T>` is call-site-compatible with the old `{success,error}` shape. **Bug fix**: `scripts/ci/check-forbidden-patterns.sh` had `set -eo pipefail` + unchecked `grep` exit-1-on-no-match silently killing the scan; wrapped in `|| true`. Forbidden-pattern baseline ratcheted 97→57 (~41% reduction). |
| Dashboard caching | ✅ Complete (Apr 15) | `apps/erp/src/lib/cached-dashboard-queries.ts` — 5 `unstable_cache` wrappers on aggregation RPCs: `getCachedPipelineSummary` (300s), `getCachedCompanyCashSummary` (600s), `getCachedAmcMonthlySummary` (900s), `getCachedProjectsWithoutTodayReport` (180s), `getCachedLeadStageCounts` (300s). Uses `createAdminClient` because unstable_cache can't wrap cookie-reading functions — safe because every wrapped function returns only company-level totals (never row-level data that could leak across users). Exported `DASHBOARD_TAGS` for future `revalidateTag()` invalidation. Wired into founder-dashboard.tsx (4 of 7 Promise.all calls) + leads-pipeline-queries.ts (delegation). Expected ~60% DB round-trip reduction on dashboard loads. |
| Migration 050 | ✅ Applied (dev, Apr 14) | **Inverter telemetry infrastructure.** 7 new tables: `inverters` master (6-brand CHECK, polling interval, current_status), `inverter_monitoring_credentials` (vault secret refs only, never raw), `inverter_readings` + `inverter_string_readings` **partitioned monthly by RANGE(recorded_at)** — 6 partitions live (2026-04/05/06 × 2 parents). `inverter_readings_hourly` + `_daily` rollups with partial index `(day, PR) WHERE PR<0.70`. `inverter_poll_failures` audit. **8 functions**: `get_inverters_due_for_poll(batch_limit)`, `create_inverter_partition_for_month()`, `rollup_inverter_readings_hourly()` / `_daily()` (recompute last 2 days for late-arriving), `drop_old_inverter_partitions()` (90-day with safety check on rollup health), **`create_service_tickets_from_inverter_alerts()`** — scans daily rollup for PR<0.70/offline>60min/fault>0 anomalies, creates TKT-NNNN in om_service_tickets with 7-day dedup window. **Feeds directly into Service Tickets V3 module — no new UI needed.** **5 pg_cron schedules** active: monthly partition creator (`0 3 28 * *`), nightly hourly rollup (`17 2 * * *`), nightly daily rollup (`22 2 * * *`), 90-day retention (`42 3 * * *`), daily auto-ticket (`1 7 * * *`). Types regen +565 lines. Ready to accept readings today; live polling awaits Sungrow/Growatt API registration. |
| packages/inverter-adapters | ✅ Complete (Apr 14) | New workspace package at `packages/inverter-adapters/`. `base.ts` — `InverterAdapter` interface, `NormalizedReading` (SI kilo units, nullable, JSONB raw_payload for vendor specifics), `NormalizedStringReading`, `AdapterCredentials` bag, error classes (`AdapterError`, `InvalidCredentialsError`, `NotImplementedError`) carrying httpStatus+payloadExcerpt for the poll failures log, `syntheticReading(capacity)` generator (IST hour-of-day solar curve, ±10% jitter). Per-brand adapters: sungrow, growatt, sma, huawei — all validate credentials, map vendor status → NormalizedStatus, return synthetic when `SYNTHETIC_INVERTER_READINGS=1` env var is set, throw NotImplementedError for live path with API call sequence in TODO comments. `factory.ts`: `getAdapter(brand)`. **Edge Function `supabase/functions/inverter-poll/index.ts`** — Deno endpoint polling inverters on schedule, dispatches adapters, upserts readings with `ON CONFLICT (inverter_id, recorded_at) DO NOTHING`, updates inverter health, logs failures + marks polled on error so next cycle doesn't hammer. Fail-per-inverter — one bad device can't kill the batch. |
| Playwright smoke tests | ✅ Complete (Apr 15) | @playwright/test installed in apps/erp. `playwright.config.ts` (30s timeout, headless Chromium, auto-start dev server locally). 5 tests in `e2e/smoke.spec.ts`: /login renders, founder dashboard after login, /leads, /projects, /price-book. Tests 2–5 `test.skip()` when PLAYWRIGHT_LOGIN_EMAIL/_PASSWORD env vars absent — CI stays green without secrets. Every test calls `expectNoDevErrorOverlay` (fails on Next.js `data-nextjs-dialog-overlay`) — catches server-render errors previously only found via manual clicking. Separate `e2e/tsconfig.json` so Playwright globals don't pollute main tsc. Scripts: `test:e2e` + `test:e2e:install`. `pnpm test:e2e --list` discovers 5 tests. **Not wired into CI yet** — needs dev Supabase test user + GitHub Actions secrets. |
| God component splits | ✅ Complete (Apr 15) | 3-part refactor series. All 3 files >1000 LOC from the audit broken into 17 smaller modules under the 500-LOC rule #14. (1) `survey-form.tsx` 1,191→5 modules at `components/projects/forms/survey-form/` (types.ts 199, shared.tsx 315, sections-primary.tsx 436, sections-secondary.tsx 344, index.tsx 445). (2) `project-files.tsx` 1,124→6 modules at `components/projects/project-files/` (types.ts 118, helpers.ts 152, parts-rows.tsx 282, parts-boxes.tsx 223, generated-docs.tsx 135, index.tsx 434). (3) `proposal-wizard.tsx` 1,024→6 modules at `components/proposals/proposal-wizard/` (shared.tsx 156, step-lead-system.tsx 232, step-bom.tsx 215, step-payment.tsx 149, step-review.tsx 199, index.tsx 281). All 3 import paths unchanged — monolith.tsx → monolith/index.tsx — so all callers untouched. Pure refactors, zero runtime changes, behavior preserved byte-for-byte. |
| Migration 054 | ✅ Applied (dev, Apr 15) | **Storage RLS perf fix.** Replaces inline `EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() ...)` on 8 storage.objects policies with `public.get_my_role() = ANY(ARRAY[...])`. `get_my_role()` was defined in migration 008a as `STABLE + SECURITY DEFINER` so Postgres caches within a single statement — a file listing of 100 rows now hits profiles once per statement instead of per row. Covers project-files (read/insert/update/delete) + site-photos (read/insert/update/delete — **UPDATE policy added, was missing** from migration 019 same way project-files was before migration 047; would have caused .move() within site-photos to fail silently). Role lists preserved byte-for-byte. proposal-files intentionally skipped — owned by marketing revamp branch (migration 052 adds marketing_manager to those policies). Not the JWT-hook approach originally proposed in the audit (would need dashboard config + token re-issue across all active sessions — ops risk). STABLE-helper captures ~90% of the win with zero config change. Verified via pg_policies — all 8 policies show new pattern. |
...
| Migration 045 | ✅ Applied (dev) | ceig_scope column on net_metering_applications (shiroi\|client), engineer_signature_path on commissioning_reports. Drives CEIG scope toggle + commissioning signature capture. |
| Migration 046 | ✅ Applied (dev) | Price Book expansion: vendor_name, default_qty, deleted_at, rate_updated_at, rate_updated_by + 24-category CHECK constraint + indexes. |
| Migration 047 | ✅ Applied (dev) | `project_files_update` RLS policy on `storage.objects` (founder/project_manager/site_supervisor) — fills the gap left by migration 010, which only created SELECT/INSERT/DELETE. Required because Supabase Storage `.move()` is implemented as an UPDATE on `storage.objects`. |
| Documents drag-drop fix | ✅ Complete | Drag-and-drop file recategorization on the project detail Documents tab was failing for every file with "Move failed - Object not found". Root cause was a missing UPDATE RLS policy on the `project-files` bucket — Supabase surfaces a denied UPDATE as "Object not found" because the row becomes invisible to the post-update visibility check. Migration 047 adds the policy. No client code change required. |
| Projects module 3-bug fix | ✅ Complete | Manivel reported three major issues blocking daily PM work: BOM not auto-pricing from Price Book, no PDF downloads working anywhere in Projects, CEIG not showing for ≥10 kWp projects. Three distinct root causes, all fixed in one pass. tsc --noEmit: 0 errors. See entries below. |
| BOM auto-pricing fix | ✅ Complete | `applyPriceBookRates` in `project-step-actions.ts` used exact `${category}::${description}.toLowerCase()` match as its only strategy. BOM descriptions are free-form user text (often imported from legacy Excel); Price Book descriptions are Manivel's curated entries (e.g., "Waree 540Wp Mono PERC WSMD-540"). Exact matches essentially never happened → 0 items were ever priced. Rewrote with 4-strategy layered matching: normalized-exact (collapse whitespace + strip punctuation + lowercase), substring (either direction), Jaccard token overlap ≥0.3 (tokenize both into word sets, score by intersection/union, pick best candidate in category), single-candidate fallback (if category has exactly one priced entry, use it). Added skipped-count logging for debugging. |
| Server PDF render fix | ✅ Complete | `apps/erp/next.config.js` had NO `serverComponentsExternalPackages` config. `@react-pdf/renderer` v4.3.2 pulls in fontkit, pdfkit, and linebreak — all of which use dynamic `require()` of font files and binary resources. Webpack cannot statically bundle them for Vercel serverless functions, so every server-side `renderToBuffer()` call in the PDF API routes (`/api/projects/[id]/survey`, `/api/projects/[id]/qc/[inspectionId]`, `/api/projects/[id]/dc/[dcId]`, `/api/projects/[id]/commissioning`, `/api/procurement/[poId]/pdf`) was failing silently with an opaque 500. Added `experimental.serverComponentsExternalPackages: ['@react-pdf/renderer']` — Next.js 14.2.29 uses the `experimental.` key (top-level `serverExternalPackages` was only added in Next 15). No route handler code change required. **Requires a Vercel redeploy to take effect.** |
| CEIG ≥10kW gate fix | ✅ Complete | `step-liaison.tsx` line 126 had `showCeig = application.ceig_required \|\| (system_size_kwp > 10 && system_type !== 'on_grid')`. The `!== 'on_grid'` clause was exactly backwards — on-grid is precisely the system type that needs CEIG, because CEIG is the regulatory gate for TNEB net metering. The check was HIDING CEIG for the main Shiroi use case (grid-connected commercial/industrial ≥10 kWp). Also the `> 10` was off-by-one — TN's regulatory cutoff is `>= 10`. Changed to `sizeKwp >= 10 && systemType !== 'off_grid'` — includes on-grid + hybrid, excludes only off-grid (which doesn't touch TNEB). CEIG card + workflow stage now render correctly. |
| Survey PDF | ✅ Complete | Full survey report PDF via @react-pdf/renderer — project details, roof specs, electrical, shading, signatures, embedded photos. API route `/api/projects/[id]/survey/pdf`. Download button on stepper survey step. |
| BOQ qty inline edit | ✅ Complete | Double-click quantity cell in project BOQ table. updateBoqItemQuantity recalculates total_price automatically. |
| DC PDF null fix | ✅ Complete | Delivery challan PDF crashed on items with missing fields. Defensive null guards in delivery-challan-pdf.tsx + API route projections. |
| Execution task visibility | ✅ Complete | Project execution step shows all tasks — tasks without milestone appear in new "Other Tasks" group below the 10 milestones. |
| Liaison CEIG scope | ✅ Complete | Projects ≥10kW show Shiroi/Client toggle on liaison step. "Managed by Client" card hides CEIG form when client handles liaison. |
| SignaturePad | ✅ Complete | Reusable HTML5 Canvas signature component — touch + mouse input, undo, clear, data URL export. |
| Commissioning signatures | ✅ Complete | Commissioning report captures engineer + client digital signatures via SignaturePad. Uploads to project-files bucket, embeds actual images in finalized PDF. |
| Task corrections | ✅ Complete | Removed strikethrough on completed tasks, fixed project dropdown to show all projects (not just active), filter shows only projects with tasks. |
| AMC Module V4 | ✅ Complete | Manivel's 9-column table restructure: Project Name clickable, Category Free/Paid, Scheduled Visits X/Y expandable, Status toggle, Next AMC Date, Completed Date, Notes, Actions, Report. Enhanced getAllAmcData: second om_visit_schedules query + client-side grouping computes visit stats per contract. getProjectsWithAmc for filter. |
| Service Tickets V3 | ✅ Complete | 3-digit ticket numbers (TKT-001, TKT-002 via padStart), customer_name-only clickable green link, SearchableProjectFilter + getProjectsWithTickets query (uses om_service_tickets table). |
| PO rate inline edit | ✅ Complete | Double-click rate cell on PO detail. updatePoLineItemRate fetches qty+gst_rate, recalculates total_price, then recalculates PO subtotal+gst+total. Fixed pre-existing item.quantity → item.quantity_ordered bug. |
| PO PDF + download | ✅ Complete | Purchase Order PDF via @react-pdf/renderer — Shiroi green brand bar, company header with GSTIN, PURCHASE ORDER title, 5-cell info strip, Vendor\|Ship-To party block, items table with HSN, CGST/SGST 50/50 split for intra-state TN, round-off, authorized signature, page numbers. API route `/api/procurement/[poId]/pdf`. Download button + Cancel PO soft-delete (status=cancelled) on PO detail. |
| Price Book CRUD | ✅ Complete | 7 server actions: paginated getPriceBookItems with search/category/brand/vendor filters, createPriceBookItem, updatePriceBookItem (auto rate_updated_at+rate_updated_by audit), deletePriceBookItem (soft via deleted_at), getPriceBookCategories/Brands/Vendors for filter dropdowns. |
| Price Book page V2 | ✅ Complete | Full /price-book rewrite: sticky filter bar (search + category + brand + vendor), 9-column table (S.No, Category, Item, Make, Qty, Unit, Rate/Unit with inline edit, Vendor, Actions), pagination 50/page, Add/Edit/Delete dialogs, "Rate pending" amber badge for zero-rate items. |
| Price Book Sheets import | ✅ Complete | **217 items imported direct from Manivel's master sheet** via the existing `shiroi-migaration` service account (already wired into gdrive sync scripts — no CSV export required). New `scripts/import-price-book-from-gdrive.ts` uses Sheets API, flexible header matching, dry-run default, `--commit` to write. CATEGORY_MAP extended with Manivel's spellings (`Conduits`, `Misscellaneous`). **Final state:** 252 active rows (35 seed + 217 import), 22 categories, 22 brands, 17 vendors, 48 rate-pending. Legacy `scripts/import-price-book-csv.ts` kept as offline-CSV fallback. |
...
| Migration 051 | ✅ Applied (dev, Apr 15) | **Marketing + Design revamp — enum additions.** `app_role` gains `marketing_manager` (Prem's dedicated role). `lead_status` gains 4 new stages: `quick_quote_sent` (after contacted, Path A), `design_in_progress` (after site_survey_done), `detailed_proposal_sent` (after design_confirmed), `closure_soon` (after negotiation, pre-win gate). Split from migration 052 because Postgres 17 forbids referencing a newly-added enum value in the same transaction that adds it. |
| Migration 052 | ✅ Applied (dev, Apr 15) | **Marketing + Design revamp — schema, triggers, RLS.** Extended `channel_partners.partner_type` CHECK with `consultant`/`referral`/`electrical_contractor`/`architect`/`mep_firm`. Extended `tasks.category` CHECK with `payment_followup`/`payment_escalation`. New `leads` columns: `channel_partner_id` FK, `consultant_commission_amount`, `consultant_commission_locked_at/by`, `base_quote_price`, `design_confirmed_at/by`, `design_notes`, `draft_proposal_id`. New `price_book_id` FK on `proposal_bom_lines` + `project_boq_items` (sync chain Quote→BOQ→PO). New `followup_sla_days` + `escalation_sla_days` on `proposal_payment_schedule`. New tables: `lead_closure_approvals` (amber-band founder approvals), `consultant_commission_payouts` (per-tranche disbursements tracked to customer_payments). **Triggers:** replaced `create_payment_followup_tasks()` with per-milestone SLA version assigning to marketing_manager (with fallback to project_manager); new `fn_lock_consultant_commission_on_partner_assignment()` BEFORE UPDATE on leads; new `fn_create_consultant_payout_on_customer_payment()` AFTER INSERT on customer_payments with 5% TDS deduction; new `fn_migrate_lead_files_to_project()` AFTER INSERT on projects renames `proposal-files/leads/<id>/**` → `project-files/projects/<id>/**` via `storage.objects` UPDATE. New function `enqueue_payment_escalations()` for the hourly pg_cron job. **RLS:** `marketing_manager` gains full CRUD on leads/proposals/bom_lines/payment_schedule/channel_partners/net_metering_applications/lead_closure_approvals/consultant_commission_payouts, read-only on projects. `designer` gains SELECT on projects + full access on price_book. `project_manager` downgraded to SELECT-only on `net_metering_applications`. Storage RLS rebuilt for `proposal-files` bucket (insert/update/delete for marketing_manager + designer + existing roles). |
| Migration 053 | ✅ Applied (dev, Apr 15) | **Marketing + Design revamp — seed + data migration.** Remapped 61 existing leads from `proposal_sent` to `detailed_proposal_sent`. Seeded per-milestone SLAs on all existing `proposal_payment_schedule` rows via CASE on `due_trigger` (on_acceptance 3/4, on_material_delivery 5/9, mid_installation 7/7, on_commissioning 7/7, after_net_metering 14/14, retention_period_end 30/30, custom 7/7). Fuzzy-matched `price_book_id` on existing BOM lines via Jaccard-like tokenized overlap within same item_category (threshold 0.25) — 23/35,022 BOM lines matched + 1/673 BOQ items matched (legacy data is mostly too free-text to match Manivel's curated price book, which is expected). New proposals created post-migration enforce `price_book_id` via code. |
| Marketing + Design lib layer | ✅ Complete (Apr 15) | `leads-helpers.ts` extended VALID_TRANSITIONS for all 4 new stages (Path A quick route, Path B detailed route with design_in_progress + detailed_proposal_sent, closure_soon gate), DEFAULT_PROBABILITY entries, new `STAGE_LABELS` Record export (single source of truth). `budgetary-quote.ts` extended with optional `preferredBrands` steering hook ({ panel: 'Waree', inverter: 'Sungrow' }). `proposal-actions.ts` `createBudgetaryQuoteAction` now seeds per-milestone `followup_sla_days`/`escalation_sla_days` on inline payment schedule. New `quote-actions.ts` (545 LOC): `createDraftDetailedProposal(leadId)` pre-creates draft on site_survey_scheduled entry + stashes FK on lead, `finalizeDetailedProposal(proposalId)` one-click finalizer (validates every BOM line has price_book_id, recomputes totals against live rates, sets `base_quote_price` on lead which triggers commission lock, flips lead to `detailed_proposal_sent`), `escalateQuickToDetailed(leadId)` for Path A→B conversion, plus `addBomLineFromPriceBook`/`removeBomLine`/`updateBomLineQuantity` for the price-book-gated BOM editor. New `partners-queries.ts` + `partners-actions.ts` (pair) with paginated list, detail, linked leads, pending/paid payouts, FY Apr 1 aware summary, full CRUD + `assignPartnerToLead` (triggers DB commission lock) + `markPayoutPaid`. New `closure-actions.ts` + `closure-queries.ts`: `classifyBand` (green ≥10%, amber 8-10%, red <8%), `computeMargin` (live BOM-sourced formula, consultant commission excluded), `attemptWon` (green→flip, amber→insert lead_closure_approvals + founder notifications, red→block), `approveClosure`/`rejectClosure` (founder-only), `listPendingClosureApprovals` (joined with leads + employees for display). New `payment-followups-queries.ts`: `getPaymentFollowups()` + `getPaymentFollowupsSummary()` (total_open/overdue/escalated). `roles.ts` extended with `marketing_manager` role entry in ROLE_LABELS + full SECTIONS_BY_ROLE (Sales/Design/Liaison/Payments/Projects R-O/Reference/Contacts), new `/sales` and `/partners` ITEMS entries, founder nav updated to use `ITEMS.sales` + `ITEMS.partners`, `designer` role gains read-only windows onto Sales + Projects. `lead-status-badge.tsx` + `status-change.tsx` + `lead-tabs.tsx` reconciled with new enum values + URL-space adaptive. |
| Marketing + Design UI | ✅ Complete (Apr 15) | **Routes:** `/sales` list page with extended `lead-stage-nav` (Path A quick / Path B detailed / closure_soon inline, colored section borders); `/sales/[id]/*` full subtree (new, layout, detail, activities, tasks, files, payments, proposal) as thin re-exports of `/leads/[id]/*`. `/partners` HubSpot-style list (filters by partner_type/active/search, commission rate formatting, TDS badges). `/partners/[id]` detail (4 KPI cards with FY YTD, Contact + Commission info boxes, Pending Payouts with inline `MarkPayoutPaidButton` client component, Leads-from-partner table, Recent Paid history). **Closure UI:** `ClosureBandBadge` + `ClosureBandHelper` in `/components/sales/closure-band-badge.tsx`; `AttemptWonButton` client component calls `attemptWon` server action with live band branching; closure banner wired into `leads/[id]/layout.tsx` — when `lead.status === 'closure_soon'` calls `computeMargin()` and renders amber-band Card with Base/BOM cost/Site est breakdown + live badge + AttemptWonButton. **Founder approvals:** `ClosureApprovalsPanel` server component (drop-in for dashboard) shows pending amber-band requests with inline `ClosureApprovalActions` client component. **Payments Follow-ups:** `PaymentFollowupsTable` component with summary cards (open/overdue/escalated) + 7-col table + `MarkFollowupCompleteButton` wrapper around existing `toggleTaskStatus`; wired into `/payments` as new "Follow-ups" filter tab. **Liaison rehoming:** `step-liaison.tsx` accepts `readOnly?: boolean` prop — when true, wraps content in `pointer-events-none select-none`, shows amber banner "Managed by Marketing — read-only", hides `LiaisonCreateButton`; `/projects/[id]` liaison case passes `readOnly={viewerRole === 'project_manager'}` so Manivel sees status but can't edit. **Middleware:** `/leads` + `/leads/*` → 307 redirect to `/sales` + `/sales/*` (preserves sub-tab path), `/proposals` → `/sales`. `tsc --noEmit`: 0 errors. |
| Migration 056 | ✅ Applied (dev, Apr 15) | **Second FK bug — `log_proposal_status_change`.** Same bug pattern as 055 but on proposals side. Dormant until 055 added `trg_mark_proposal_accepted_on_lead_won` which cascades a proposal UPDATE on every lead→won transition; that UPDATE fired `log_proposal_status_change`, which writes `auth.uid()::UUID` (profile.id) into a column that's FK to `employees.id`, and the chain rolled back. My 055 verification missed this because MCP runs as service role where `auth.uid()=NULL` so the NULL fallback handled it silently. Real users have non-null `auth.uid()` which fails the FK. Rewrote `log_proposal_status_change` to look up `employees.id` via `profile_id = auth.uid()` with NULL fallback — same fix pattern as migrations 031 (`log_project_status_change`) and 055 (`log_lead_status_change`). Re-verified end-to-end by wrapping the test in `BEGIN; SET LOCAL request.jwt.claim.sub = '27b71db9-...'; SET LOCAL role = authenticated; DO $$ ... $$; ROLLBACK;` — confirms `auth.uid()=27b71db9-..., lead_history=1→2, proposal_history=0→1, proposal=accepted, projects=0→1`. **Also**: fixed `column-config.ts` LEAD_COLUMNS.status options which were out of sync with the stepper (had legacy `proposal_sent`/`converted`/`disqualified`, missing `quick_quote_sent`/`design_in_progress`/`detailed_proposal_sent`/`closure_soon`). Replaced with the exact 13 stages from `lead-stage-nav.tsx` STAGE_ORDER. Inline-edit on /sales now matches the detail page dropdown. |
| Migration 055 | ✅ Applied (dev, Apr 15) | **Three bug fixes found during Marketing + Design user testing.** (1) **FK violation on any lead status change.** `log_lead_status_change()` was writing `auth.uid()` (which is `profiles.id`) directly into `lead_status_history.changed_by`, but that column is FK to `employees.id` — two different IDs. Every lead-status UPDATE failed. Same bug pattern that migration 031 fixed for `log_project_status_change`. Rewrote to look up `employees.id` via `profile_id = auth.uid()` with NULL fallback (column was made nullable in migration 012 for system ops). (2) **Won → project cascade missing.** Flipping a lead to `won` via dropdown or `attemptWon` only updated `leads` — the related proposal stayed `draft`/`sent`, so the existing `create_project_from_accepted_proposal` trigger never fired, no project was auto-created. New AFTER UPDATE trigger `trg_mark_proposal_accepted_on_lead_won` finds the most recent in-play proposal (detailed preferred over budgetary, most recent wins), marks it `accepted`, which cascades into the existing proposal trigger → project spawns. Works from any lead-status path. If no in-play proposal exists, logs a NOTICE and lets the `won` transition through (PM creates project manually). (3) **Latent `employees.deleted_at` bugs in migration 052.** `create_payment_followup_tasks()` v2 and `enqueue_payment_escalations()` both filtered on `e.deleted_at IS NULL` but `employees` has `is_active BOOLEAN`, not `deleted_at`. Latent until a project status change fired the trigger or the hourly cron ran. Rewrote both to use `e.is_active = TRUE`. **Verification:** DO block with RAISE EXCEPTION rollback on a real detailed-proposal lead confirmed the full chain end-to-end: `lead_status_history.count=1→2`, `proposals.status=sent→accepted`, `projects.count=0→1`. Real data untouched. Also tightened `leads-helpers.ts` `VALID_TRANSITIONS` to only offer stepper-visible destinations — removed `disqualified`, `converted`, `proposal_sent` from dropdown options so users see only the 13 stages on the stage-bar nav. |
| Marketing + Design follow-ups | ✅ Complete (Apr 15, commit 2118bb6) | **Three items landed after the initial UI:** (1) **ClosureApprovalsPanel mounted on founder dashboard** — import + JSX placement in FounderDashboard above the main 3-col grid; self-hides via `null` return when no pending approvals. (2) **`/design/[leadId]` per-lead workspace rewritten end-to-end.** New `design-actions.ts` with `submitDesignConfirmation` (BOM+notes validation, flips to design_confirmed, stamps metadata), `saveDesignNotes` (blur-save), `sendBackToDesign`. New `components/design/lead-files-panel.tsx` — drag-drop category grid scoped to `proposal-files/leads/{leadId}/{category}/`, reuses `CategoryBox` + `PhotoSlideshow` from project-files/parts-boxes for one visual pattern, 6 categories (drawings/pvsyst/photos/specs/proposal/misc). New `components/sales/bom-picker.tsx` — shared price-book-gated BOM editor used by both design workspace and Quote tab, searchable picker over live `price_book` with 50-result cap + category/brand/description filter, qty input, inline row qty edit, Trash2 remove, warning chip for legacy free-text lines, all mutations via quote-actions.ts. New `components/design/design-notes-editor.tsx` — textarea with blur-save + "Mark Design Confirmed" button with inline blocker-reasons list (disabled until BOM has lines, every line has price_book_id, notes non-empty, lead in design_in_progress). Page auto-creates draft detailed proposal for Path B leads. Survey summary read-only card from `lead_site_surveys`. (3) **Quote tab rebuilt** (`/leads/[id]/proposal/page.tsx`). New `components/sales/consultant-picker.tsx` — searchable select over active `channel_partners`, assigns via `assignPartnerToLead` which fires the DB commission lock trigger, shows locked amount + TDS badge, unassign clears the lock. New `components/sales/finalize-detailed-proposal-button.tsx` — one-click `finalizeDetailedProposal` caller. Page has 5 sections: quick-action bar with link to design workspace + `QuickQuoteButton`, ConsultantPicker, draft-proposal `BomPicker` for Path B, "Send Detailed Proposal" card with green/amber requirements checklist + FinalizeDetailedProposalButton when ready, historical proposals list with Quick Quote / Detailed / Active Draft badges. **Pre-existing bug fix:** was selecting `total_price` + `margin_pct` from `proposals` (don't exist on the schema), now uses `total_after_discount` + `gross_margin_pct`. **`tsc --noEmit`: 0 errors.** Remaining: production deployment of migrations 051/052/053 after employee testing week. |
| Migration 058 | ✅ Applied (dev, Apr 15) | **Category standardisation dedup fix.** Migration 057 included a `price_book` dedup step keyed by `(item_description, item_category)` then created a unique index on the same tuple. The dedup logic used incorrect normalisation and left residual duplicate rows that blocked unique-index creation. 058 re-runs the dedup using the correct normalisation (trim + lowercase + collapse internal whitespace on `item_description`) then builds the unique index successfully. Strictly a follow-up/fix to 057 — no new business logic. Local filename collision with the plant-monitoring migration (also initially numbered 058) resolved by renaming the latter to `059_plant_monitoring.sql` — Supabase tracks by timestamp so both applied cleanly; only the local filename was ambiguous. |
| Migration 059 | ✅ Applied (dev, Apr 16) | **Plant Monitoring module (O&M).** New `plant_monitoring_credentials` table (multi-entry-per-project, soft delete, 14 cols, 5 indexes incl. partial unique `(project_id, portal_url) WHERE deleted_at IS NULL`). New `plant_monitoring_detect_brand(TEXT)` IMMUTABLE helper that classifies a portal URL into sungrow/growatt/sma/huawei/fronius/solis/other via lowercase substring match. New AFTER UPDATE trigger `fn_sync_plant_monitoring_from_commissioning()` on `commissioning_reports` — fires when status transitions to submitted/finalized AND all three monitoring fields are non-null; upserts via ON CONFLICT on the unique index so re-submissions refresh credentials instead of duplicating. New RPC `get_plant_monitoring_summary()` (STABLE SECURITY INVOKER) — single-statement aggregation returning total count + per-brand counts + missing-credentials count (projects with finalized commissioning but no live creds row). BEFORE UPDATE trigger maintains `updated_at`. RLS with `get_my_role() = ANY(ARRAY[...::app_role])`: founder + project_manager full CRUD, om_technician SELECT only, no physical DELETE policy = soft delete only. Types regenerated (+106 lines). Drives new `/om/plant-monitoring` page (sticky filter bar, 3 summary cards, 7-column table with per-row password show/hide + 30s auto-remask + copy-to-clipboard). |
...
| Dev schema status (Apr 16) | ✅ Current | All 59 migrations (001 through 059) applied on dev Supabase (`actqtzoxjilqnldnacqz`) — verified via MCP `list_migrations` + spot-check of 13 key objects (plant_monitoring_credentials table, inverters + inverter_readings partitioned telemetry, data_flags, project_bois, channel_partners, lead_closure_approvals, and 8 RPCs: get_pipeline_summary / get_my_role / get_plant_monitoring_summary / plant_monitoring_detect_brand / get_company_cash_summary / get_lead_stage_counts / get_amc_monthly_summary / get_projects_without_today_report, plus the marketing_manager + closure_soon enum literals). No pending migrations on dev. |
| Prod deployment | 🔜 Waiting | Prod (`kfkydkwycgijvexqiysc`) still on a much older schema. All migrations 013–059 are "dev-only, prod pending". No migrations have been promoted to prod since the last coordinated window. Will batch-promote after Manivel's employee testing week on dev completes — at that point clone dev schema to prod + selectively migrate data. |
...
**Migrations:** 59 files (001 through 059)
**Last updated:** April 16, 2026 (v3.22 — Plant Monitoring O&M module shipped end-to-end per Manivel's spec. Migration 059 (renamed from 058 to resolve filename collision with the Apr 15 category-standardisation dedup fix). New `plant_monitoring_credentials` table with AFTER UPDATE trigger on `commissioning_reports` that auto-syncs credentials on status transition to submitted/finalized, URL-based brand detection helper, summary RPC, RLS gated to founder+project_manager (write) + om_technician (read). UI: `/om/plant-monitoring` with 7-column table, per-row password eye-toggle with 30s auto-remask + copy-to-clipboard, Add/Edit/Delete dialogs, project + brand + search filters, pagination. All discipline gates pass (type-check, lint, forbidden-pattern baseline unchanged at 61, Playwright smoke test 6/6 discovered). 11 commits across 9 tasks. **Dev schema status reconciled:** all 59 migrations (001 through 059) applied on dev Supabase — verified via MCP `list_migrations`. Prod still on older schema; batch promotion waits on employee testing week. v3.21 notes follow:) April 14, 2026 (v3.21 — Projects module 3-bug batch fix. Manivel reported three blocking issues in a single message: BOM not auto-pricing from Price Book, no PDF downloads working anywhere in Projects, CEIG workflow not appearing for ≥10 kWp projects. Systematic-debugging Phase 1 found three distinct root causes. **(1) BOM auto-pricing:** `applyPriceBookRates` used exact `${category}::${description}.toLowerCase()` match as its only strategy — BOM descriptions are free-form and Price Book descriptions are curated, so the strings never matched. Rewrote with 4-strategy layered matching (normalized-exact → substring → Jaccard token overlap ≥0.3 → single-candidate fallback). **(2) Server PDF render:** `next.config.js` had no `serverComponentsExternalPackages` entry. `@react-pdf/renderer` v4.3.2 depends on fontkit/pdfkit/linebreak which use dynamic `require()` of font files — webpack cannot statically bundle these for Vercel serverless functions. Every server-side PDF route (survey, QC, DC, commissioning, PO) was failing with an opaque 500. Added `experimental.serverComponentsExternalPackages: ['@react-pdf/renderer']` (Next 14.2.29 uses the `experimental.` key). Requires Vercel redeploy to take effect. **(3) CEIG ≥10kW gate:** `step-liaison.tsx` had `showCeig = ... (system_size_kwp > 10 && system_type !== 'on_grid')` — the `!== 'on_grid'` was exactly backwards (on-grid is precisely the type that needs CEIG), and `> 10` was off-by-one (TN rule is `>= 10`). Changed to `sizeKwp >= 10 && systemType !== 'off_grid'`. tsc --noEmit: 0 errors. 3 files changed.)

**Earlier on April 14, 2026** (v3.20 — Documents tab drag-drop fix. Manivel reported "Move failed - Object not found" on every drag-and-drop file recategorization. Phase 1 systematic-debugging traced the issue to a missing UPDATE RLS policy on `storage.objects` for the `project-files` bucket. Migration 010 had only created SELECT/INSERT/DELETE policies, but Supabase Storage `.move()` is implemented as an UPDATE on `storage.objects` — denied UPDATEs surface to the client as "Object not found" because the row becomes invisible to the post-update visibility check. Migration 047 adds `project_files_update` mirroring INSERT (founder/project_manager/site_supervisor). No client code change required. Verified via `pg_policies`: SELECT/INSERT/UPDATE/DELETE all in place.)

**Earlier on April 14, 2026** (v3.19 — Manivel PM Corrections fully delivered including the 217-item Price Book import via direct Google Sheets pull. Used the existing `shiroi-migaration@shiroi-migataion.iam.gserviceaccount.com` service account (already configured for gdrive sync scripts) to read Manivel's master sheet directly — no CSV export step. New `scripts/import-price-book-from-gdrive.ts` with flexible header matching + dry-run default. CATEGORY_MAP extended for Manivel's spellings. Final `price_book` state: 252 active rows = 35 seed + 217 import, 22 categories, 22 brands, 17 vendors, 48 rate-pending.)

**Earlier on April 14, 2026** (v3.18 — Manivel PM Corrections, all 4 batches complete. **Batch A — Project Module:** Migration 045 (ceig_scope + engineer_signature_path), Survey PDF + stepper download, BOQ qty inline edit, DC PDF null fix, execution "Other Tasks" group, Liaison CEIG Shiroi/Client toggle, reusable SignaturePad component, commissioning digital signatures (engineer + client). **Batch B — Task/AMC/Tickets:** Task strikethrough fix + all-projects dropdowns + tasks-only filter, AMC Module V4 (9-column table + computed visit stats + Next AMC / Completed dates), Service Tickets V3 (3-digit numbers + customer_name links + SearchableProjectFilter). **Batch C — Purchase Orders:** PO rate inline edit with auto-recalc, full PO PDF template + API route, Download PDF + Cancel PO action bar. **Batch D — Price Book:** Migration 046 (vendor_name, default_qty, soft delete, rate audit, 24-category CHECK), price-book-actions.ts CRUD, page overhaul with sticky filters + pagination + inline rate edit + "Rate pending" badge, CSV import script ready (awaiting founder to export private Google Sheet). 23 commits across the 4 batches, 0 TypeScript errors.)
