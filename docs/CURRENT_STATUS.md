...
> **Last updated: 2026-07-30** — Manivel/Vivek feedback sweep shipped to dev (mig 218, spec `2026-07-30-purchase-bom-ticket-amc-fixes-design.md`): Purchase nav "Projects"→"Budgets"; BOM wrap-text across `/bom-review` (now ListPageShell + sticky thead, reads extracted to `bom-review-queries.ts`), proposal BOM and project BOM tab; `formatDate()` made timestamp-tolerant, which fixed Service-Ticket **Created** and AMC **Completed Date** showing "Invalid Date" plus ~25 latent sites repo-wide; new `/om/amc/[id]` contract detail page (visit cards, report view/download, per-visit Work Activity via `om_visit_events`, per-visit delete, paid-AMC Service Amount) and an AMC project autosearch filter. Two other sessions also shipped to dev today: expenses list-page rework (mig 217) and the ticket-number atomic-sequence fix (mig 215) — note migration numbers 214 and 215 each got claimed twice by concurrent sessions; this work took 218 to stay clear. **Still waiting on Vivek:** flip the Vercel function region iad1→icn1 (biggest remaining perf win) + Micro→Small trial; decide on the pre-existing `site_supervisor` price leak via the projects BOQ/BOM tabs. Earlier in-flight unchanged — BOI-Manager parity purchase flow (migs 210/211/213): `/purchase` workspace + instant PO/PDF + `/purchase/orders` + `/purchase/projects` rollup + `/purchase/intake` (price-stripped site-engineer app). Open items: site_supervisor can still see prices via the projects BOQ/BOM tabs (pre-existing leak, needs product decision); ₹ glyph unavailable in react-pdf builtin Helvetica → PDFs print "Rs." (repo-wide font fix running as separate task); Manivel's sheet data not yet imported into the ERP tables (import decision pending). Prior in-flight (digest v3 mig 205, Zoho sync mig 204) unchanged: — merged PRs #14/#13/#10/#11 + docs PRs #9/#6/#4 to main. Founder 7AM digest v3 (mig 205, dev): work-done lines carry lead/project names, new Won-yesterday + Due-before-today sections — n8n workflow 19 JSON updated in-repo, needs a `scripts/push-n8n-workflows.ts` push to update the live fallback. Zoho Books live API sync (mig 204, dev; inbound pull + voucher-only write-back; `/settings/zoho-sync`) — awaiting Vivek's Zoho self-client creds + edge-fn/n8n deploy for go-live. Spec `2026-07-16-zoho-live-api-sync-design.md`.
