...
> **Last updated: 2026-09-07** — Speed: Vercel function region flipped iad1→**icn1** in-repo (`apps/erp/vercel.json`, preview verified `regions:["icn1"]`), removing the ~0.9 s-per-DB-round-trip Washington↔Seoul tax behind the “ERP is too slow to use” complaint (before: projects list 5.1 s, project details 2.65 s, stepper tabs 2.0–2.6 s). Usage had collapsed — Supabase edge logs show only ~2–20 ERP requests/day from Vercel in the first week of September. Next levers, in order: Micro→Small DB trial (cold-plan tax, now the dominant server cost), then the project-detail/stepper waterfalls (each tab = auth→profile→header→step data in series). Still open from 2026-07-30: site_supervisor price leak via projects BOQ/BOM tabs; ₹ glyph in react-pdf; Manivel sheet import decision; Zoho sync go-live creds; BOI purchase flow (migs 210/211/213) unchanged.
