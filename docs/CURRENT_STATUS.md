...
> Last updated: **April 20, 2026** (Tier 1 n8n scaffolding complete on `feat/n8n-workflow-scaffolding` — 13 more workflow JSONs (11 webhook + 2 cron), router expanded to 16 routes, 5 more `emitErpEvent` sites wired, migration 085 with 8 more digest views applied to dev, types regen + strip-script added. Zoho data-accuracy pass earlier on the same branch with mig 084 DDL already live; cash-position root-cause fix migs 080+081 before that).
