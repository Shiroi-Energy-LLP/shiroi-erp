...
> Last updated: **April 20, 2026** (migs 082+083 applied to dev via Supabase MCP, types regenerated, `emitExpenseSubmitted` swapped to `whatsapp_number` — n8n scaffolding ready to import workflows; Zoho data-accuracy pass earlier on the same branch with mig 084 DDL already live; cash-position root-cause fix migs 080+081 before that).
...
| **n8n workflow scaffolding — `feat/n8n-workflow-scaffolding`** | Claude (overnight 2026-04-18/19; migs applied 2026-04-20) | 🔄 Awaits morning review | Branch has 10 commits ready for Vivek to review + merge. Foundation for the full 59-workflow catalog. **Commits (in order):** LLP legal-name corrections across 8 files · `emitErpEvent()` helper in `apps/erp/src/lib/n8n/emit.ts` + `N8N_EVENT_BUS_URL` env · migration 082 `employees.whatsapp_number` · event-bus router workflow JSON (`00-event-bus-router.json`) + global error handler (`55-global-error-handler.json`) + programmatic push script (`scripts/push-n8n-workflows.ts`) · 4 Tier 1 sub-workflow templates (lead-created, proposal-approved, project-commissioned, expense-submitted — all with SIMULATED SEND placeholder until WABA lands) · emit wired into `advanceProjectStatus` (fires `project.commissioned` on completed) and `submitExpense` (fires `expense_claim.submitted` with enriched payload) · migration 083 7 digest views (`v_digest_*` — Tier 2 source data) · WhatsApp template catalog `infrastructure/n8n/templates.md` (~20 templates, UTILITY + MARKETING) · `turbo.json` env-var declarations · **migs 082+083 applied to dev via Supabase MCP, types regenerated, `emitExpenseSubmitted` swapped `personal_phone` → `whatsapp_number`, 083 pre-apply fix: `v_digest_projects_overdue_commissioning` now filters on dates (`actual_end_date` set + `commissioned_date` null + >30d old) since `project_status` enum has no `'installed'` value.** Type-check + lint clean across the whole repo. No files merged to main yet. **To unblock morning: (1)** merge branch; **(2)** finish WABA creation and create the Header Auth (`x-webhook-secret`) + Gmail OAuth (`Gmail (Vivek)`) n8n credentials; **(3)** run `pnpm tsx scripts/push-n8n-workflows.ts` to import all six workflows (router + error handler + 4 Tier 1 subs). Migration state on dev: 081 tracked + 082+083 tracked + 084 DDL live (applied outside `supabase_migrations.schema_migrations`, columns confirmed nullable) — counts already reflect the Zoho full-re-run. |
...
|-----|---------------|---------|
| **Dev** (`actqtzoxjilqnldnacqz`) | **083** (digest views, Apr 20 via Supabase MCP) — 082 (whatsapp_number) also applied same run. Mig 084 (nullable project_id) DDL is live on dev tables but was applied outside `supabase_migrations.schema_migrations`; Zoho re-run overnight relied on it. | None blocking n8n work — 084 not in migrations table but its ALTERs are in effect |
| **Prod** (`kfkydkwycgijvexqiysc`) | 012 (approximate — last coordinated window) | **013 through 083** — 71 migrations waiting on the next prod window |
