...
> Last updated: **April 20, 2026** (Zoho data-accuracy pass across phases 06-12 + mig 084 overnight on `feat/n8n-workflow-scaffolding`; cash-position root-cause fix earlier, migs 080+081; n8n workflow scaffolding also pending review on this branch).
...
| **n8n workflow scaffolding — `feat/n8n-workflow-scaffolding`** | Claude (overnight 2026-04-18/19) | 🔄 Awaits morning review | Branch has 9 commits ready for Vivek to review + merge. Foundation for the full 59-workflow catalog. **Commits (in order):** LLP legal-name corrections across 8 files · `emitErpEvent()` helper in `apps/erp/src/lib/n8n/emit.ts` + `N8N_EVENT_BUS_URL` env · migration 082 `employees.whatsapp_number` (NOT YET APPLIED — run in SQL Editor + regen types) · event-bus router workflow JSON (`00-event-bus-router.json`) + global error handler (`55-global-error-handler.json`) + programmatic push script (`scripts/push-n8n-workflows.ts`) · 4 Tier 1 sub-workflow templates (lead-created, proposal-approved, project-commissioned, expense-submitted — all with SIMULATED SEND placeholder until WABA lands) · emit wired into `advanceProjectStatus` (fires `project.commissioned` on completed) and `submitExpense` (fires `expense_claim.submitted` with enriched payload) · migration 083 7 digest views (`v_digest_*` — Tier 2 source data; NOT YET APPLIED) · WhatsApp template catalog `infrastructure/n8n/templates.md` (~20 templates, UTILITY + MARKETING) · `turbo.json` env-var declarations. Type-check + lint clean across the whole repo. No files merged to main yet. **To unblock morning: (1)** apply migrations 082 + 083 to dev Supabase, regen `packages/types/database.ts`, swap `personal_phone` → `whatsapp_number` in `emitExpenseSubmitted` enrichment; **(2)** merge branch; **(3)** finish WABA creation and create the Header Auth + Gmail OAuth n8n credentials; **(4)** run `pnpm tsx scripts/push-n8n-workflows.ts` to import all six workflows (router + error handler + 4 Tier 1 subs). |
