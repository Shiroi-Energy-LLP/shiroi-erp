...
> **Last updated: 2026-09-07** — Speed: Vercel function region flipped iad1→**icn1** in-repo (`apps/erp/vercel.json`, preview verified `regions:["icn1"]`), removing the ~0.9 s-per-DB-round-trip Washington↔Seoul tax behind the “ERP is too slow to use” complaint (before: projects list 5.1 s, project details 2.65 s, stepper tabs 2.0–2.6 s). Usage had collapsed — Supabase edge logs show only ~2–20 ERP requests/day from Vercel in the first week of September. **Pages are still 1.7–4.5 s after the flip** — the remaining cost is cold paths on every click (PostgREST pool idles out after ~30 s → 0.9–3.6 s to reopen backends; cold plans; serial auth hops; Vercel Hobby instance variance) — measured breakdown + fix options in §6 of `reviews/2026-07-19-erp-speed-full-report.md`. Same day, Vivek approved + shipped: **mig 222** keep-warm cron (pg_cron+pg_net, 8 pings/20 s; idle burst 1.2–3.6 s → 0.45–0.76 s) and `auth.getClaims()` in middleware + `getAuthUser` (no GoTrue hop per navigation; revoked sessions live ≤1 h). Remaining levers: Micro→Small DB trial (Vivek), per-tab RPC bundling (BOQ first: `get_project_boq_items_masked` 532 ms cold), Vercel Hobby variance (watch). Still open from 2026-07-30: site_supervisor price leak via projects BOQ/BOM tabs; ₹ glyph in react-pdf; Manivel sheet import decision; Zoho sync go-live creds; BOI purchase flow (migs 210/211/213) unchanged.
...
| **Page-load perf audit** (`docs/reviews/2026-06-19-page-load-perf-audit.md` + `2026-07-19-erp-speed-full-report.md`) | Dev-only. June: mig 190 (pg_trgm), [G1] session `cache()`, migs 192/194/195/196 RPCs, om G5. July 19: mig 206 (RLS initplan wrap, 185 warns→0), mig 207 (`list_bucket_objects` kills storage.search fan-out), mig 208 (price-book facets), mig 209 (43 redundant indexes dropped), G5 finished (/tasks, /my-tasks, /activities, /price-book, proposal editor). **2026-09-07: region flip iad1→icn1 SHIPPED via `apps/erp/vercel.json` (was pending since July). Still pending: Micro→Small trial + picker smoke-test.** Reconciliation-page rewrite still open. |
...
- ~~**Vercel function region flip `iad1` → `icn1`**~~ — DONE 2026-09-07 in-repo (`apps/erp/vercel.json` `regions:["icn1"]`; overrides the dashboard default). If the DB ever moves (prod in Mumbai), change this file to `bom1` in the same PR.
...
|-----|---------------|---------|
| **Dev** (`actqtzoxjilqnldnacqz`) | **222** (2026-09-07 keep-warm cron; 218 = 2026-07-30; 209 = 2026-07-19 — perf batch: 206 RLS initplan wrap, 207 storage-listing RPC, 208 price-book facets, 209 redundant-index drop; 205 = 2026-07-17 digest v3) | None |
