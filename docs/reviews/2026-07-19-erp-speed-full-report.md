...
**Shipped the same evening (Vivek approved):** cause #1 → **mig 222** `postgrest_keep_warm()` (pg_cron `20 seconds` → 8 async `net.http_get` pings; secrets in Vault; 3-day purge of `cron.job_run_details`). Re-measured with the cron active: 10 parallel after 50 s idle = **0.45–0.76 s** (was 1.2–3.6 s); 1 request after idle from a Chennai laptop still ~1 s because that path's Cloudflare colo (MAA) is not the one the pings warm (ICN) — the ERP's functions are in icn1, so they get the warm path. Cause #3 → `auth.getClaims()` in `packages/supabase/src/middleware.ts` + `apps/erp/src/lib/auth.ts` `getAuthUser` (local ES256 verify; ~120 other direct `auth.getUser()` calls in server actions are per-action, not per-navigation, and were left alone). Rollback for either is in the migration header / a one-line revert.

**Hygiene done today:** `pg_stat_statements` reset at 11:27 UTC (P2-2), so future means reflect current behaviour.

**Post-deploy check (12:10 UTC):** login intact; edge logs show no `/auth/v1/user` per navigation any more (one cached `jwks.json` fetch, 121 ms). Warm RSC fetch per tab: details/survey/execution **0.52–0.60 s**, BOQ 1.35 s. What is left is (a) **Vercel instance cold starts** — five sequential HTML requests landed on five different instances; time-to-first-byte ranged 0.3 s (warm) to 3.9 s (cold) with identical code and a warm DB, so a user can still see a 4–6 s tab on an unlucky click (Hobby plan; only the plan or a smaller bundle changes this) — and (b) the **BOQ tab's own work**, 0.75–2.2 s of data + render after first byte (10 parallel queries incl. `get_project_boq_items_masked`) → bundle into one RPC next.
