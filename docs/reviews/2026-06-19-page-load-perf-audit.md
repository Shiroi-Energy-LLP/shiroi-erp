...
**Still open (real refactors — need a dedicated, runtime-verified pass):**
- **[G5] lazy-load eager dialog/dropdown data.** Bigger than it looks: the "eager" lists feed *both* a closed dialog *and* the table/page — `/tasks` 478-project list → `CreateTaskDialog` **and** `TasksTable` per-row edit; also `/om/inverters` (1,000 projects + creds), `/om/amc` (~700 + employees), `/price-book` (3 facet scans), `/sales/[id]/proposal` (full `price_book`). Each needs *both* consumers rewired to fetch on dialog-open — it touches core create/edit flows and its correctness (does the dropdown populate on open?) is **runtime-only**, not build-checkable, so it wants a pass with the app running, not a blind edit.
- **Bigger JS-aggregation rewrites:** the reconciliation page (3 unbounded full-table selects + JS join, ships the whole set to the client → needs a summary RPC + pagination), `/procurement` list `getPurchaseRequests` (2 unbounded `.in()` fan-outs + JS money → parameterized GROUP BY RPC), `/om/profitability` KPI totals (low-impact: JS sum over already-aggregated per-project rows).
