...
- **[G6] `getProjectFinancials` → `get_project_financials` RPC (mig 195).** The FinancialBox rollup (contracted value + BOQ total + approved site expenses + margin) was 3 parallel reads + a JS `.reduce`; now one SQL pass. Cross-checked on dev vs raw recompute. SECURITY INVOKER; `database.ts` regenerated in-commit.
- **`getMSMEAlertPOs` SQL filter.** Was `SELECT *` + 2 embeds + unbounded then `.filter(is_msme)` in JS; now an `!inner` vendor join + `.eq('vendors.is_msme', true)` filters in SQL (no wasted non-MSME rows).
- **[G6] `getPurchaseRequests` → `get_purchase_request_aggregates(uuid[])` RPC (mig 196).** The `/procurement` list paged 50 projects then fired 2 unbounded `.in()` reads (`project_boq_items` + `purchase_orders`) + JS loops; now one GROUP-BY pass per page. Cross-checked on dev (amounts + PO counts match raw recompute). SECURITY INVOKER.
- **[G5] `/om/inverters` + `/om/amc` lazy-load.** Both create-dialogs now fetch their project pickers on first open (inverters ≤1,000 projects; amc ≤200 + ≤500) instead of on every page render; the pages no longer eager-load them. Build-verified. *Smoke-test: open each create-dialog, confirm the project picker populates.*

**Still open (need a runtime-verified pass or a larger rewrite):**
- **[G5] remaining pages.** `/tasks` — 478 projects feed **both** `CreateTaskDialog` and the per-row `EditTaskDialog`; a 5-file, 2-dialog change across core task CRUD (proven pattern, but not blind-shippable while AFK). `/activities`, `/price-book`, `/sales/[id]/proposal` are **always-visible** filter/picker controls (not dialog-open), so they need a combobox-on-focus fetch, not the dialog pattern. All need the app running to confirm the control still populates.
- **Bigger rewrites:** reconciliation page (3 unbounded full-table selects + JS join, ships the whole set to the client → summary RPC + pagination); `/om/profitability` KPI totals (low-impact: JS sum over pre-aggregated rows).
- **Low-value cleanups:** `/qc-gates` + `/daily-reports` `SELECT *` trims + NEVER-DO #15 inline-query moves (both already `.limit(100)`-bounded).
