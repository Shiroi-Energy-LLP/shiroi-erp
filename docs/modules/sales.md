...
- **(mig 179 → superseded by mig 200) Lead close date is status-driven** — the BEFORE-UPDATE trigger `fn_set_lead_close_date_on_status` (fires `BEFORE UPDATE OF status`) is centralized so every status path (inline-edit, stage nav, `bulkChangeLeadStatus`, detail) behaves identically — **don't replicate this in app code.** **mig 200 (2026-06-28) split the actual close date out of the forecast:** `expected_close_date` is now **forecast-only** (so the "closing when" filter never shows Won deals); the real Won date lives in the new **`leads.closed_date`** column (indexed `idx_leads_closed_date`). On entering **won** the trigger stamps `closed_date = today (IST)`, **clears** `expected_close_date` (mig 179 used to *stamp* it here — reversed) **and** `next_followup_date`, and soft-completes the lead's open `lead_followup` task. Leaving won (reopen) clears `closed_date`. **on_hold / lost** still clear `expected_close_date` (mig 179, unchanged). Legacy `converted` / `disqualified` untouched; INSERTs aren't guarded (imports set their own dates). Ordered alphabetically after `trg_block_lead_won_without_proposal`, so a blocked won never touches dates. `closed_date` is exposed in `search_leads_by_query`, as a (default-off) "Closed Date" toggle column, and on lead detail/header ("Closed On" / "Closed"). mig-200 backfill: 385 Won leads — `closed_date` from the old `expected_close_date` stamp (else most-recent `lead_status_history` 'won' date; none invented), then `expected_close_date` + `next_followup_date` cleared.
- **Editing the lead name** — corrected via the pencil button beside the `<h1>` in `leads/[id]/layout.tsx`, which opens `lead-name-edit-dialog.tsx` → `renameLead(leadId, newName)` in `leads-actions.ts` (trims, rejects empty, caps 150 chars, zero-rows guard, revalidates detail + breadcrumb + both list tables). Rename lives on the detail page by design. **(mig 172)** The leads table's `linkField` is now the computed **`customer_project`** ("Customer — Project"), not `customer_name`. `customer_name` is now a non-default toggle column and, since it is no longer the link, IS inline-editable when shown (writes via `updateCellValue`, which bypasses `renameLead`'s 150-char cap — prefer the detail-page pencil for renames). New sibling columns: `company_name` (display-only) + `project_name` (inline-editable). The combined label = `company.name ?? customer_name` (+ ` — project_name`), computed in the query layer (`leads-queries.ts`, both the PostgREST and search-RPC paths) via the pure `formatCustomerProject` helper (`apps/erp/src/lib/customer-project.ts`).
- **(mig 178) Proposals "Customer — Project" + saved-views backfill.** `PROPOSAL_COLUMNS` now defaults to a combined **`customer_project`** column (customer_name demoted to a toggle), computed in `getProposals` via `formatCustomerProject` from the linked lead. The `search_proposals` RPC was **DROP+recreated** to also return `lead_company_name` + `lead_project_name` (LEFT JOIN `companies` via `leads.company_id`) — the return shape changed, so `CREATE OR REPLACE` alone won't do and types must be regenerated. Mig 178 also **backfilled every `table_views` row** (all users · leads/projects/proposals) `customer_name → customer_project` in place (idempotent, order-preserving): a saved view's stored `columns` override the config default, so the default columns from mig 172 weren't enough on their own. **Caveat:** the proposals list (`/proposals`) is still 307'd to `/sales` (see Middleware gotcha next), so the proposals column/view changes are correct but not user-visible until that list is un-retired.
...
- **Follow-up ↔ task two-way sync** — `leads.next_followup_date` (the leads-list "Next Follow-up" column) and the lead's open `lead_followup` task are mirrored in **both** directions. **Lead→task:** trigger `trg_sync_lead_followup_task` (mig 108) + the idempotent `upsertLeadFollowupTask(leadId, dueDate)` action (called from `status-change.tsx` after a status UPDATE and from `inline-edit-actions.ts` on inline date edit; `addLeadActivity` sets the date directly) materialize/update the single open `lead_followup` task. **Task→lead:** trigger `trg_sync_followup_task_to_lead` (mig 193; **broadened to ANY task type in mig 197**) sets `next_followup_date` to the earliest open lead task of **any category** — Call / Site-visit / Document / Payment / Follow-up (NULL when none remain) — whenever a lead task is created / rescheduled / completed / soft-deleted. Reps log their real next step via the `QuickAddTask` **Type** dropdown, so the list date follows whichever open task is soonest, not only the Follow-up-typed one. (Mig 193 was `lead_followup`-only: once the rep completed the follow-up task the date froze in the past while a later Call/Document task sat open — the drift Vivek hit on ~7 dev leads, 2026-06-21. Mig 197 also re-ran the reconcile over **all** open lead tasks, disabling the lead→task trigger during the backfill so filling a date from a Call/Document task never spawns a phantom `lead_followup` task.) Both triggers short-circuit on `pg_trigger_depth() > 1`, so the two never recurse into each other. Action-path failure is non-fatal (logged, not surfaced). **(mig 200)** the task→lead direction now skips leads with `status = 'won'`, so an open Call/Document task left on a closed deal can't resurrect `next_followup_date` after the Won trigger cleared it.
...
- `docs/superpowers/specs/2026-06-09-leads-projects-customer-project-name-design.md` — leads (+ projects) optionally link to a `companies` record + carry `project_name`; combined "Customer — Project" default column on /leads, /sales, /projects (`company.name ?? customer_name` + ` — project`); reusable `CompanyPicker` (pick/create-inline) on the New-Lead form + `CompanyProjectEditor` on lead & project detail; **mig 172** adds `project_name` to both tables, makes `create_project_from_accepted_proposal` carry `company_id`+`project_name` to the spawned project, and recreates `search_leads_by_query` to return + match company name + project_name. Phase 2 (backfill existing rows into companies) deferred to its own spec.
