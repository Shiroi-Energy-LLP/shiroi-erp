...
- **Follow-up ↔ task two-way sync** — `leads.next_followup_date` (the leads-list "Next Follow-up" column) and the lead's open `lead_followup` task are mirrored in **both** directions. **Lead→task:** trigger `trg_sync_lead_followup_task` (mig 108) + the idempotent `upsertLeadFollowupTask(leadId, dueDate)` action (called from `status-change.tsx` after a status UPDATE and from `inline-edit-actions.ts` on inline date edit; `addLeadActivity` sets the date directly) materialize/update the single open `lead_followup` task. **Task→lead:** trigger `trg_sync_followup_task_to_lead` (**mig 193**) sets `next_followup_date` to the earliest open `lead_followup` task (NULL when none remain) whenever such a task is created / rescheduled / completed / soft-deleted — this is what makes a follow-up task added in the lead's Tasks tab (`QuickAddTask`, now defaulting to type **Follow-up**) move the list date. Both triggers short-circuit on `pg_trigger_depth() > 1`, so the two never recurse into each other. Action-path failure is non-fatal (logged, not surfaced).
