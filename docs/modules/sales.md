...
- **(mig 179) Lead close date is status-driven** — a BEFORE-UPDATE trigger (`fn_set_lead_close_date_on_status`, fires `BEFORE UPDATE OF status`) stamps `expected_close_date = today (IST)` when a lead enters **won**, and clears it when a lead enters **on_hold** / **lost**. Centralized so every status path (inline-edit, stage nav, `bulkChangeLeadStatus`, detail) behaves identically — **don't replicate this in app code.** Only the user-driven statuses are handled (legacy `converted` / `disqualified` are untouched); INSERTs aren't guarded (imports set their own dates, same reasoning as the won-block trigger). It mutates `NEW` directly (no extra write) and is alphabetically ordered after `trg_block_lead_won_without_proposal`, so a blocked won never stamps a date. Won leads with no `lead_status_history` 'won' row were left as-is by the mig-179 backfill (no date invented).
