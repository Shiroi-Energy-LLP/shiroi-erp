...
- **Out-of-range `recorded_at` → insert fails:** `inverter_readings` is range-partitioned by month; a reading whose `recorded_at` (e.g. a device's stale or future `last_update_time`) falls outside every existing partition throws `no partition of relation "inverter_readings" found for row`. **Resolved 2026-06-07:** the Edge Function now calls `clampRecordedAt()` before every upsert — any timestamp older than 3 days or more than 1h in the future is clamped to `now()` so it lands in the current partition. pg_cron still pre-creates partitions monthly as the primary mechanism; the clamp is the safety net.
- **n8n "connection was aborted, perhaps the server is offline" on workflow #60:** this is the n8n HTTP Request node timing out (not a real outage) — the Edge Function keeps running and still logs a `200`, but n8n already gave up and marked the cron failed. Diagnose via the **edge-function logs** (`get_logs service=edge-function`): if `execution_time_ms` exceeds the node's `timeout`, that's the cause. **Resolved 2026-06-07:** the function is now time-bounded (`FETCH_TIMEOUT_MS=10s` per vendor call via `fetchWithTimeout`, `POLL_BUDGET_MS=30s` cumulative; over-budget inverters defer to the next cycle) so it always returns well under the n8n timeout, which was also raised 55s → 90s for headroom. Sequential processing of all ~50 inverters with no per-call timeout was what let one hung vendor endpoint blow past the limit.
