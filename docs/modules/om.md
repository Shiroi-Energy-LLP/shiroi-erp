...
- `/om/tickets` — service ticket list (ticket numbers still mint internally as TKT-NNNN but are **no longer displayed** — column removed 2026-06-11 per Vivek; `created_at` auto-records and shows as Created)
  ...
  - **"Done By" is an explicit picker (2026-07-25).** It still stores to `resolved_by`, but is now chosen by the user in both create + detail edit, because the auto-stamp recorded whoever clicked the toggle rather than the technician who did the work. The status auto-stamp became **fill-if-empty** (only lands when `resolved_by` is null), an explicit `doneBy` always wins within one save, and **reopening no longer nulls `resolved_by`** (erasing a deliberately entered name was data loss — clear it via the picker instead). `resolved_at`/`closed_at` still clear on reopen.
  - Edit (pencil) **navigates to `/om/tickets/[id]` (2026-07-25)** — the quick-edit dialog was deleted; the detail panel is a strict superset and the two had already drifted (the dialog couldn't set project, status or done-by). One edit surface.
  - **Delete = hard DELETE (2026-06-27)** behind a confirm — permanently removes the ticket, cascades `om_ticket_events`, best-effort `project-files` Storage cleanup; needs the new `tickets_delete` RLS policy (founder/PM/om_technician). Closing a ticket stays a *status* via the inline toggle.
  ...
  - Create dialog: project picked via the searchable `ProjectCombobox` (customer – project name, number badge) — replaced the plain dropdown 2026-06-11. **Header parity (2026-07-25):** also sets Status, Done By, Service Amount and a back-dated **Created Date** (`createdAt` `'YYYY-MM-DD'`, stamped 06:30 UTC = IST midday so it can't render as the previous day), so an already-completed paid service is one round trip instead of create-then-edit. Creating straight into resolved/closed runs the same `applyStatusTransition` stamping (no closed row with a null `resolved_at`) and fires `om_ticket.resolved` alongside `om_ticket.created`, since no status transition will ever fire for it. The dialog now imports `ISSUE_TYPES`/`SEVERITY_OPTIONS` from `ticket-constants.ts` rather than keeping private copies.
...
- **AMC contract numbering (rewritten 2026-07-30, mig 221)**: `AMC-FREE-0001` / `AMC-PAID-0001`, **one independent sequence per category**, stamped by the `om_contracts_set_contract_number` BEFORE INSERT trigger → `next_amc_contract_number(amc_category)`. **Never pass `contract_number` from application code** — the trigger overwrites whatever the caller supplies (verified: an insert passing `'CALLER-SUPPLIED-GARBAGE'` came back `AMC-PAID-0006`). **Why a trigger and not mig 215's column DEFAULT:** a Postgres DEFAULT expression cannot read other columns of the row being inserted, so it cannot branch on `amc_category`; the column keeps a `DEFAULT ''` placeholder purely so `contract_number` generates as optional in `database.ts` and callers can omit it. An unknown/NULL `amc_category` **raises** rather than defaulting into the FREE series (`amc_category` is still nullable — mig 044 added it without NOT NULL, and a CHECK passes on NULL). Until mig 221, `createAmcSchedule` had the same three defects mig 215 fixed for tickets: read-then-insert (not atomic), seeded from `ORDER BY created_at DESC LIMIT 1` instead of MAX(serial), and `split('-').pop()` — which 005d's own documented format `AMC-PROJ-087-Y1` would have turned into `AMC-FREE-NaN`. The concurrency hole had two shapes: same category → duplicate key on `om_contracts_contract_number_key`; **different** categories → no error at all and two contracts silently sharing a serial. Nothing was actually corrupt when this was found (9 clean rows on one shared counter, so each series was gapped: FREE = 1,2,3,4,8 / PAID = 5,6,7,9) — this was prevention, not recovery. Vivek's call: split the counters and renumber, so mig 221 renumbered the 9 rows chronologically **per category** → `AMC-FREE-0001..0005` + `AMC-PAID-0001..0004` (5 of 9 user-visible numbers changed). Safe to renumber: every reference to a contract is by `contract_id` (UUID), `contract_number` is stored as text nowhere else in the schema, and — unlike tickets — there is no RAG ingest for contracts, so no index needed refreshing. `updated_at` was preserved (trigger parked during the renumber — a renumber is not a user edit).
...
- `om_contracts` (`contract_number` AMC-FREE-0001 / AMC-PAID-0001 style — stamped by the `om_contracts_set_contract_number` BEFORE INSERT trigger off one sequence per category, never set by app code (mig 221); `amc_category`, `amc_duration_months`, `annual_value`)
...
- Migration 221 (`om_amc_free_contract_number_seq` + `om_amc_paid_contract_number_seq` + `next_amc_contract_number(p_category)` + `om_contracts_set_contract_number` BEFORE INSERT trigger; renumbered 9 contracts per category; TS generator deleted from `createAmcSchedule`)
