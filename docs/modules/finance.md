...
## Zoho Books Live API Sync (July 16, 2026 — mig 203, spec `2026-07-16-zoho-live-api-sync-design.md`)
...
- `zoho_sync_queue` — outbound sync queue, **vouchers (expenses) only since mig 203**; `status` enum: pending/syncing/synced/failed/skipped (the earlier pending/in_progress/done/dead description matched the stale mig-072 RPCs, dropped in 203); claimed via `claim_zoho_voucher_batch()` with SKIP LOCKED.
- `zoho_sync_state` / `zoho_sync_runs` — inbound live-sync watermarks per Zoho module + run log (mig 203). Surfaced at `/settings/zoho-sync`.
