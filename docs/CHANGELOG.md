...
- **[2026-07-30]** fix(finance/payments): Sentry `JAVASCRIPT-NEXTJS-12` — "Failed to load payment schedule follow-ups … Could not find a relationship between 'proposals' and 'projects'". `getPaymentScheduleFollowUps` hinted `projects!projects_lead_id_fkey`, a sibling hop via `leads` that PostgREST can't embed → hard PGRST200. Now `projects_proposal_id_fkey` (UNIQUE ⇒ object, 1:1, 508/508 vs the lead hop's 557 fan-out). Never worked — bad hint was in its own original commit 40c05d2. → no migration · module: finance

- **[2026-07-30]** fix(om/amc): om_technician could read the AMC screens but not write them — `om_schedules_write`/`om_contracts_write` were `FOR ALL` founder+PM only (mig 005d), so visit status/reschedule/assign, work-done fields, report upload, the Open/Closed toggle and Create AMC all failed **silently** (RLS = 0 rows affected, not an error). Both tables realigned to this module's own `tickets_update` convention (founder+PM+om_technician), with explicit INSERT/UPDATE/DELETE policies replacing `FOR ALL`. Also swapped mig 218's bare `auth.uid()` for `get_my_role()` (restores mig 206's initplan-0) and covered the `om_visit_events.created_by` FK. Proof: impersonated an om_technician under RLS in a rolled-back txn → visit UPDATE 1, contract UPDATE 1, event INSERT 1. → mig 220 (dev) · spec `2026-07-30-purchase-bom-ticket-amc-fixes-design.md` · module: om
...
- **[2026-07-25]** data(purchase): Price Book refreshed from `Price List - 26-27.xlsx` (dev) — 199 rows repriced (56 real movements), 15 new items, 12 zero-reference April duplicates retired; 24 non-material rows (IC/MMS/transport) left as-is. Script `scripts/import-price-list-2026-27.ts` (dry-run default, idempotent). → review `2026-07-25-price-book-fy26-27-import.md` · module: purchase
- **[2026-07-21]** feat(purchase): BOI-Manager parity purchase flow — `/purchase` workspace (BOI table, inline edit, status-money KPIs, bulk bar, multi-select → instant PO + minimal PDF to project-files), `/purchase/orders` log (edit meta / delete w/ revert), `/purchase/projects` rollup (budget vs BOI vs expenses vs profit %), `/purchase/intake` price-stripped site-engineer app, with/without-price CSV+PDF exports. → migs 210/211/213 (dev) · spec `2026-07-20-purchase-flow-boi-manager-spec.md` · module: purchase
- **[2026-07-20]** feat(projects): /projects list — Expected Start / Expected Completion (reuse planned_* dates) + Next Follow-up columns (hidden by default, inline-editable), multi-select status filter, status sort puts completed last (generated `status_rank`), `next_followup_date` trigger-syncs an open `project_followup` task to the PM (mirrors lead mig 108). → mig 212 (dev; renumbered from 210 — purchase-flow session took 210/211) · spec `2026-07-20-projects-list-dates-followup-multistatus-design.md` · module: projects
- **[2026-07-19]** perf(db): speed-report P0/P1 batch — mig 206 wraps bare `auth.*()` in ~190 RLS policies in `(SELECT …)` (kills per-row re-eval, 185 advisor warns → 0); mig 207 `list_bucket_objects` RPC (indexed C-collation range over storage.objects, SECURITY INVOKER) replaces the storage.search fan-out (Documents tab 33→2 calls, design panel 7→1, + 4 flat listers); mig 208 `get_price_book_facets` (3 full-table dropdown fetches → 1 RPC); mig 209 drops 43 provably-redundant indexes (prefix-covered + backup tables). → migs 206–209 (dev) · `docs/reviews/2026-07-19-erp-speed-full-report.md`
- **[2026-07-19]** perf(g5): remaining lazy-load sweep — `SearchableProjectSelect` self-fetches on first open (create/edit-task dialogs, My Tasks quick-add; `/tasks` + `/my-tasks` no longer eager-load ~480 projects), Add Activity dialog lazy-loads its picker (`getActivityProjectOptions`), `/sales/[id]/proposal` fetches price_book only when the BOM editor renders. SMOKE-TEST pending: open each picker once. · same report
- **[2026-07-19]** docs(perf): ERP speed full report (code + infra) — root cause is topology: Vercel functions in `iad1` (US East) × Supabase Seoul × Chennai users ≈ 1.2–2.5 s TTFB; plan = flip function region to `icn1`, RLS initplan migration (185 advisors), Micro→Small trial, storage `.list()` de-load; droplet exonerated. → `docs/reviews/2026-07-19-erp-speed-full-report.md`
- **[2026-07-19]** feat(ui): promote hand-rolled ERP patterns into `@repo/ui` — new `KpiCard` (promoted from app, +`trend.positiveIsGood`/`'flat'`), `InfoBox` (info/success/warning/error callout w/ 3px left accent), `StageIndicator` (routing-free stepper), `FieldHint` (non-RHF form hint/error), `CardLabel`; additive props `Badge.dot`, `Label.required`, `Card.interactive` (all default-off). Wired real usages: KPI import repoint across all dashboards + command-center/reconciliation (deleted app `kpi-card.tsx`), InfoBox callouts (proposal-gate-banner, leads/[id]/layout, orphan-banner), Card interactive + CardLabel (payments, pipeline-summary), Badge dot (vendors), FieldHint (financial-box). No migration; CI green (types/lint/build 90/90). → module: design-system
- **[2026-07-19]** fix(ui): repoint stale green shadcn tokens in `packages/ui/src/globals.css` (`--primary`/`--accent`/`--ring` + their `-foreground`, both `:root` and `.dark`) from pre-rebrand green `145 100% 35%` to Solar Gold `37 100% 44%` / ink `38 55% 8%`, matching `apps/erp/src/app/globals.css`. Zero runtime effect — the packages/ui globals is exported but unimported (app loads its own already-gold globals); this is design-system-reference hygiene so `@repo/ui/globals.css` + the design-sync bundle stop showing the abandoned brand hue. Neutrals left as-is. · module: ui
...
- **[2026-07-17]** feat(sales): founder 7AM digest v3 per Vivek — "Work done 24h" lines now carry the lead/project customer name (new `v_digest_tasks_done_24h`; bare titles like "to follow" were meaningless), plus new "Won yesterday" (`v_digest_leads_won_yesterday`) and "Due before today" (reuses `v_digest_leads_overdue`) sections; "Closed last week" dropped (covered by Won yesterday + Won MTD). n8n wf-19 fallback uses the view too. → mig 205 (dev; renumbered 203→204→205 as tasks-RLS then Zoho took the earlier slots on main)
- **[2026-07-16]** feat(finance): Zoho Books **live API sync** — inbound pull of all 12 finance modules (15-min n8n cron → `zoho-sync` Edge Function, watermark-incremental, upsert on `zoho_*_id`) + outbound write restricted to **approved expense vouchers only** (8 non-voucher enqueue triggers dropped; 644 stale queue rows skipped); `/settings/zoho-sync` admin UI. Go-live needs Zoho self-client creds (spec §9). → mig 204 (dev; applied as `203_zoho_live_api_sync` pre-renumber) · spec `2026-07-16-zoho-live-api-sync-design.md` · module: finance
...
- **[2026-06-21]** feat(ui/tables): finish the sticky list-page header — after Manivel flagged that `/sales` (the real leads list behind the "Sales" nav; `/leads` is a legacy dup I'd wrongly updated first) still had the old non-merged sticky-filter, migrated `/sales` + Vendors, Inventory, Procurement (+ Orders), Partners, Price Book, Invoices, Payments/Receipts, Vendor Bills onto `ListPageShell`. New opt-in `stickyHeader` on the `@repo/ui` `Table` primitive (drops its inner `overflow-auto` box so a sticky `<thead>` anchors to the page scroll region); raw-table pages drop their `overflow-x-auto` wrapper. → no migration · modules: cross-cutting / design system
- **[2026-06-21]** feat(purchase): BOM + voucher import pipeline — upload per-project "Bill Of Materials" sheets (se-master-file layout) → tolerant `exceljs` parser (contracted + actual + voucher tracks, dynamic column anchoring, formula-total reconstruction) → pg_trgm fuzzy auto-match (`match_project_by_name` RPC) → human review/edit at `/bom-review/import` → on Confirm, `approve_bom_import` cascades into one `project_bois` + its `project_boq_items` (new `actual_quantity`/`actual_unit_price`/`actual_total_price`/`voucher_no`/`bill_status` cols). Staging table `pending_bom_imports` (founder/PM RLS); 48 sheets seeded to dev for Manivel; matches are human-reviewed, nothing blind-written; future uploads self-serve from the BOM page. → mig 198 (dev) · spec `2026-06-21-bom-voucher-import-design.md` · module: purchase
- **[2026-06-21]** feat(tasks): pinned quick-add bar on /my-tasks — always-visible inline form (Task required; optional Project via the searchable picker extracted into `searchable-project-select`; Due/Priority/Assignee defaulting to the signed-in user but reassignable) calling the existing `createTask` (project-less task = `entity_type:'project'`/`project_id:null`), rendered in the `ListPageShell` header so it stays pinned while the list scrolls. Also enlarged the shared Open/Closed `TaskStatusToggle` hit-target (`text-[10px]`→`text-xs px-2.5 py-1`) across /my-tasks + /tasks. → no migration · module: projects/tasks
- **[2026-06-21]** fix(sales): leads "Next Follow-up" date now mirrors the earliest open lead task of ANY type (Call/Site-visit/Document/Payment/Follow-up), not just `lead_followup` — reps pick the next step via the QuickAddTask **Type** dropdown, so mig 193's followup-only mirror froze the date in the past once the follow-up was completed while a later Call/Document task stayed open (~7 dev leads). Broadened `sync_followup_task_to_lead` to `MIN(due_date)` over all open lead tasks + re-ran the reconcile (lead→task trigger disabled during backfill → no phantom followup tasks); lead→task direction + depth guards unchanged; trigger-function only (no schema/type change); live trigger verified (add/complete/clear + Direction B). → mig 197 (dev) · module: sales
- **[2026-06-21]** refactor(finance/ui): shared `<InvoiceForm>` + `<PaymentForm>` behind the 4 invoice/payment dialogs (redundancy sweep §5) — create/raise-invoice + standalone/project-payment dialogs each duplicated ~120 lines of fields + GST calc; now thin wrappers over one form each, calling the canonical `createCustomerInvoice`/`recordCustomerPayment`. `PaymentForm.notify` is a REQUIRED prop so the n8n commission-emit divergence can't be wired wrong (master-ref §4.19). GST totals → whole rupees. UI-verified in the running app. → no migration · review `2026-06-18-erp-redundancy-sweep.md` §5 · module: finance
- **[2026-06-21]** style(status): unified the won/approved success green on `#16A34A` (Vivek's pick — the Solar Gold rebrand status green) — `lead-status-badge` `won` + `liaison-status-badge` `service_effected`/`approved` were the darker `#166534`; data-table was already canonical. (lead `converted` stays neutral grey — leads-domain "handed to a project", not a success state, so not a real conflict.) → no migration · modules: cross-cutting / design system
- **[2026-06-21]** refactor(perf): `/qc-gates` + `/daily-reports` inline `SELECT *` page queries → dedicated query files selecting only the displayed columns (NEVER-DO #15); forbidden-patterns baseline ratcheted 66→64. → no migration · modules: om, projects
- **[2026-06-21]** feat(ui/tables): sticky list-page header across all main lists (Projects, Leads, Proposals, Tasks, Service tickets, AMC, Activities, My Tasks, Contacts, Companies) — the filter bar stays fixed (merged flush under the topbar via `-mt`), while the page title / KPIs / View-tabs scroll away and the table's column-header row freezes. New `ListPageShell` (viewport-height flex frame + one scroll region) + `DataTable.containedScroll`; hand-rolled tables drop their `overflow-x-auto` wrapper so the sticky `<thead>` anchors to the page scroll region; `ActivitiesClient` gates it via `stickyHeader` (per-project Activities tab unchanged). Built on Manivel's Projects-page feedback. → no migration · modules: cross-cutting / design system
- **[2026-06-20]** fix(ui/nav): Import Review sidebar link rendered icon-less (blank in a collapsed sidebar, easy to miss when expanded) — `roles.ts` referenced the `Upload` icon that `sidebar.tsx` never imported or added to `ICON_MAP` (the only nav item with this gap), even though founder/PM/om_technician are all gated-in & 528 imports are pending. Now imported + mapped, so PMs like Manivel can reach `/om/import-review`. → no migration · modules: design system, om
- **[2026-06-20]** fix(tasks): /my-tasks now lists the signed-in PM's tasks + Total/Open/Closed KPI cards — the page passed `profiles.id` into `getMyTasks` (which filters `assigned_to` = `employees.id`) → always-empty list (the employee-id footgun); now resolves `getCurrentEmployeeId()` and reuses the global Tasks pipeline (FK-hint embeds) + `TasksTable` (Project/Task/Assigned/Status/Priority/Due/Notes/Done-By + Edit·Delete). → no migration · module: projects/tasks
- **[2026-06-20]** style(tables): finish the 2026-06-19 wrap sweep for Manivel — AMC Notes dropped its JS `substring(0,50)` truncation; per-project Execution Tasks Notes + Milestone blocked-reason now `whitespace-normal break-words` (were max-w-clipped / `truncate text-[10px]`). → no migration · modules: om, projects
- **[2026-06-20]** feat(ui): Shiroi sun favicon (`app/icon.svg`) — every browser tab served the generic Next.js icon (empty `public/`, no `app/icon|favicon`); Next 14 auto-wires the brand mark site-wide. → no migration · module: design system
- **[2026-06-20]** fix(sales): leads-list "Next Follow-up" date now tracks follow-up tasks created/rescheduled/completed inside a lead — new trigger `trg_sync_followup_task_to_lead` adds the missing task→lead reverse sync (mig 108 was one-way), mirroring the open `lead_followup` task ↔ `leads.next_followup_date`; both triggers depth-guarded against recursion; QuickAddTask defaults to Follow-up. → mig 193 (dev) · module: sales
- **[2026-06-20]** fix(sales/design): detailed-proposal draft now created by an explicit `StartDetailedProposalButton` click, not during render — `leads/[id]/proposal` (= `sales/[id]/proposal`) + `design/[leadId]` no longer INSERT + fire the `proposal.requested` design-head WhatsApp on a GET / prefetch / concurrent render (NEVER-DO #24). → no migration · review `2026-06-19-page-load-perf-audit.md` · modules: sales, design
- **[2026-06-20]** fix(procurement): paginate the PO list — `getPurchaseOrders` was a bare `.limit(100)` (silently 100 of ~2,046 POs); now `count:'estimated'`+`.range()`→`{rows,total}` + a pager on `/procurement/orders`. The dashboard status KPIs that shared that capped fetch now use new RPC `get_purchase_order_status_counts` (one `COUNT(*) FILTER` pass; NEVER-DO #12/#13/#25). → mig 192 (dev) · review `2026-06-19-page-load-perf-audit.md` · module: purchase
- **[2026-06-19]** refactor(employees): collapsed 5 byte-identical active-employee dropdown helpers (`getActiveEmployees` / `getActiveEmployeesForProject` / `getActiveEmployeesLite` / `listEmployeesForSelect` / `getSalesEngineers` — none filtered by role/project despite the names) onto one `lib/employees-queries.ts::getActiveEmployeesForSelect`; the five now delegate (call sites unchanged); `getSalesEngineers` returns `[]` on error now (was: threw). → no migration · review `2026-06-18-erp-redundancy-sweep.md` §3 · modules: cross-cutting
- **[2026-06-19]** refactor(finance): merged `createInvoice`/`raiseProjectInvoice` into canonical `createCustomerInvoice` (byte-identical `invoices` inserts; only the return shape differed) — thin wrappers preserve each call site's return; no n8n divergence; regression test (3). → no migration · review `2026-06-18-erp-redundancy-sweep.md` §5 · module: finance
- **[2026-06-19]** refactor(finance): merged `recordPayment`/`recordProjectPayment` into canonical `recordCustomerPayment(input, {notify})` — the ONLY divergence was the n8n `customer_payment.received` emit (salesperson commission tracking), now an explicit `notify` flag (true for /payments, false for project-detail); regression test (5). → no migration · review `2026-06-18-erp-redundancy-sweep.md` §5 + master-ref §4.19 · module: finance
- **[2026-06-19]** feat(om): Import Review "Link to existing project" action — attaches a staged plant's monitoring credential (+ one inverter row, only when the project has none AND the import has a serial) to an EXISTING project, no duplicate project (unlike Approve); new SECURITY DEFINER RPC `link_pending_import_to_project` (founder/PM, copies password ciphertext via the shared Vault key). Lets Manivel finish the O&M credential backfill from `/om/import-review`. → mig 191 (dev) · spec `2026-06-19-import-review-link-existing-project-design.md` · module: om
...
- **[2026-06-10] — docs(review): deep review #2 — security, live DB, tests, deps, n8n, a11y, DR/DPDP (no code changed).** Nine reviews incl. read-only live-dev DB work (Supabase advisors: 18 ERROR security_definer_view on digest views, 67 functions still missing SET search_path post-mig-141, 181 auth_rls_initplan perf warnings; zero FK orphans; 268 invalidated proposals). **VERIFIED-LIVE corrections to review #1:** `idx_leads_expected_close` DOES exist (retract "missing index"); n8n event-bus router is correctly wired (retract "off-by-N"). **Key new findings:** aadhaar/PAN/bank columns are plaintext `text` not pgcrypto (contradicts master-ref §13; 0 rows populated today); leaked plant credentials still in git history (commit 88a275f scrubbed tree only — needs filter-repo + rotation); `xlsx` 0.18.5 parses vendor uploads with unpatchable CVEs; `pnpm test` is RED in clean checkout (4 suites fail/unload); ~26 event-workflow WhatsApp nodes still lack executeOnce; circular backup dependency + no restore drill. → `docs/reviews/2026-06-10-deep-review-security-ops-data.md`
- **[2026-06-10] — docs(review): full codebase + module-by-module review (no code changed).** Repo-wide assessment + per-module review (all 11 modules) on three axes — house-standard adherence, UI/UX, scalability at ~10x — with prioritized suggestions, cross-module themes (stored-money float math, `as any` debt, unpaginated lists, dashboard-only alerts, quiet failures) and a 6-batch execution order. Hand-verified highlights: `po-actions.ts:82-84` persists float-computed PO totals (worst NEVER-DO #12 instance); no index exists on `leads.expected_close_date`; CI runs none of the 42 test files; May-30 `addCostVariance` bug confirmed already fixed; O&M rollup/credential discipline confirmed clean. → `docs/reviews/2026-06-10-codebase-and-module-review.md`
