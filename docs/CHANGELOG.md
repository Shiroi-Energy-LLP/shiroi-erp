...
- **[2026-04-17]** Project status crash hotfix: moving a PM-less auto-created project (e.g. Deepak) from `yet_to_start` → `in_progress` crashed the `trg_payment_followup` trigger with `null value in column created_by`. Root cause: no `marketing_manager` employee exists and `project_manager_id` was NULL → `COALESCE` fell through to NULL. Migration 062 widens the owner-resolution ladder to 3 tiers (marketing_manager → project_manager → founder). Migration 063 closes the gap at the source: `create_project_from_accepted_proposal()` now assigns the earliest-active `project_manager` (today = Manivel) as default PM on auto-create. Deepak's existing row data-patched to Manivel as PM. → migrations 062, 063
