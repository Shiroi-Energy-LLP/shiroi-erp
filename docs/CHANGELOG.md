...
- **[2026-07-20]** feat(projects): /projects list — Expected Start / Expected Completion (reuse planned_* dates) + Next Follow-up columns (hidden by default, inline-editable), multi-select status filter, status sort puts completed last (generated `status_rank`), `next_followup_date` trigger-syncs an open `project_followup` task to the PM (mirrors lead mig 108). → mig 210 (dev) · spec `2026-07-20-projects-list-dates-followup-multistatus-design.md` · module: projects
