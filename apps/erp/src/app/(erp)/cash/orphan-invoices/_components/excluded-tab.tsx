...
export function ExcludedTab() {
  ...
  const [data, setData] = useState<{ invoices: ExcludedInvoice[]; payments: ExcludedPayment[] } | null>(null);
  const [isPending, startTransition] = useTransition();
  const { addToast } = useToast();

  useEffect(() => { fetchByStatus('excluded').then((d) => setData(d as any)); }, []);
  ...
  const handleUndo = (kind: 'invoice' | 'payment', id: string) => {
    startTransition(async () => {
      ...
      addToast({ title: 'Restored — back in triage queue' });
