...
import { Wrench, CalendarCheck, MessageCircle } from 'lucide-react';
import { listPendingOutreach } from '@/lib/customer-outreach-actions';
...
export default async function OmVisitsPage() {
  ...
  // Pending customer outreach (quarterly check-ins generated by AI cron).
  // listPendingOutreach returns ActionResult — handle both branches; on failure
  // we render an empty list rather than crashing the page.
  const outreachResult = await listPendingOutreach(10);
  const pendingOutreach = outreachResult.success ? outreachResult.data : [];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pending Customer Outreach (E10 quarterly check-ins) */}
      {pendingOutreach.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <MessageCircle className="h-4 w-4 text-n-500" />
              Customer Check-ins
              <Badge variant="warning" className="ml-1">{pendingOutreach.length}</Badge>
            </CardTitle>
          </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Check-in Due</TableHead>
                  <TableHead>Days Since Commissioning</TableHead>
                  <TableHead>Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {pendingOutreach.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="text-sm text-muted-foreground">
                      {formatDate(row.checkin_due)}
                    </TableCell>
                    <TableCell className="text-sm">{row.days_since_commissioning}</TableCell>
                    <TableCell>
                      <Badge
                        variant={
                          row.status === 'sent'
                            ? 'success'
                            : row.status === 'message_generated'
                              ? 'warning'
                              : 'pending'
                        }
