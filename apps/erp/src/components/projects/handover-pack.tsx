          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-[#F5F6F8] p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-n-500 text-xs">Size</span>
                  <p>{metadata.project?.systemSizeKwp} kWp</p>
                </div>
                <div>
                  <span className="text-n-500 text-xs">Type</span>
                  <p className="capitalize">{metadata.project?.systemType?.replace(/_/g, ' ')}</p>
                </div>
                <div>
                  <span className="text-n-500 text-xs">Panels</span>
                  <p>{metadata.system?.panelBrand} × {metadata.system?.panelQuantity}</p>
                </div>
                <div>
                  <span className="text-n-500 text-xs">Inverter</span>
              ...
              <div className="space-y-1 text-sm">
                {metadata.warranty && Object.entries(metadata.warranty).map(([key, val]) => (
                  <div key={key} className="flex items-center gap-2">
                    ...
                    <span className="text-n-700">{String(val)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Checklist */}
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-n-500 mb-2">Handover Checklist</p>
            ...
            {metadata.netMetering && (
              <div className="text-sm">
                <p className="text-xs font-semibold uppercase tracking-wider text-n-500 mb-1">Net Metering</p>
                <p>
