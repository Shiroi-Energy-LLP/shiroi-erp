...
export async function StepDelivery({ projectId }: StepDeliveryProps) {
  ...
  try {
    [deliveryData, vendors] = await Promise.all([
  ...
  const hasAny = hasOutgoing || hasVendor;
      ...
      {!hasAny ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-[13px] text-[#7C818E] max-w-md text-center">
            Outgoing DCs are auto-created from the BOQ tab when items are dispatched.
            Use &quot;Upload DC&quot; above to record vendor incoming challans.
          </p>
        ...
        <>
          {/* Section 1: Outgoing Delivery Challans (from BOQ) */}
          {hasOutgoing && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  <PackageCheck className="h-4 w-4 text-p-500" />
                  <CardTitle className="text-base">Outgoing Challans</CardTitle>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-mono text-[#7C818E]">{outgoingChallans.length} DCs</span>
          ...
          {/* Section 2: Vendor Incoming Challans */}
          {hasVendor && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
          ...
          {/* Navigation to next step if no outgoing section showed the button */}
          {!hasOutgoing && (
