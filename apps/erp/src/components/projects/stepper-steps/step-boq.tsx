...
import { BoqVarianceForm } from '@/components/projects/forms/boq-variance-form';
...
export async function StepBoq({ projectId }: StepBoqProps) {
  ...
  const existingVariances = variances.map((v) => ({
    id: v.id,
  ...
  return (
    <div>
      {/* Add/Update cost entries */}
      <BoqVarianceForm projectId={projectId} existingVariances={existingVariances} />
      ...
      {variances.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-[13px] text-[#7C818E]">Click &quot;Add Cost Entry&quot; above to start tracking estimated vs actual costs.</p>
        </div>
      ) : (
        (() => {
