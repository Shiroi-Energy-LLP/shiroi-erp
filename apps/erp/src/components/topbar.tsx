    ...
    <>
      <header className="h-14 bg-white border-b border-n-200 shadow-xs flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-n-600">{profile.full_name}</span>
          <Badge variant="success">{getRoleLabel(profile.role)}</Badge>
          <button
            onClick={handleSignOut}
            className="w-9 h-9 flex items-center justify-center rounded-md text-n-600 hover:bg-n-100 transition-colors"
            title="Sign out"
          >
