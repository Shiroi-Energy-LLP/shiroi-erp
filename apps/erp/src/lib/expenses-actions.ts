...
import { canSubmitOnBehalf } from '@/lib/expenses-constants';
import { listSubmitterOptions } from '@/lib/expenses-queries';
...
export async function submitExpense(input: {
  ...
  /**
   * Submit on behalf of this employee (employees.id). Founder / *_manager /
   * Finance only — anyone else gets rejected here regardless of what the client
   * sent. Omit or pass the caller's own id for a normal self-submission.
   */
  ...
  // Delegated entry: the role gate is re-resolved server-side — never trust a
  // client-supplied role (NEVER-DO #22). RLS (mig 215) is the second gate.
  const onBehalfOf = input.submittedById && input.submittedById !== caller.employeeId
    ? input.submittedById
    : null;
  if (onBehalfOf) {
    if (!canSubmitOnBehalf(caller.role)) {
      return err('Your role cannot submit expenses on behalf of another employee');
    }
    // Validate against the same list that populates the picker — NOT a direct
    // `employees` read, which `employees_read` narrows to self + direct reports
    // for a PM/finance caller and would reject a legitimate submitter.
    const allowed = await listSubmitterOptions();
    if (!allowed.some((s) => s.id === onBehalfOf)) {
      return err('Selected submitter is not an active employee you can file for');
    }
  }

  const submittedBy = onBehalfOf ?? caller.employeeId;
  const payload: ExpenseInsert = {
    ...
    status: 'submitted',
    // submitted_by stays the claimant — it drives the voucher prefix and the
    // reimbursement. entered_by records who actually keyed it.
    submitted_by: submittedBy,
    entered_by: onBehalfOf ? caller.employeeId : null,
