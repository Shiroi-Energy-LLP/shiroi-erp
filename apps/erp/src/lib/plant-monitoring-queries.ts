...
// Shape returned by the search_plant_monitoring_credentials RPC. Cast at the
// call site until packages/types/database.ts is regenerated by the parent.
type SearchPmcRow = CredRow & {
...
export async function listPlantMonitoringCredentials(
  ...
  const rows = (data ?? []) as SearchPmcRow[];
