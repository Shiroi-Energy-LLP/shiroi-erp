...
export async function listProjectActivities(
  ...
  let query = supabase
    ...
    .select(
      'id, project_id, project_name_custom, activity_date, stage_id, stage_custom, done_by, description, se_count, os_count, contractor_count, notes, ' +
  ...
  const rows: ProjectActivityRow[] = sliced.map((r: any) => {
    ...
    return {
      ...
      project_name_custom: r.project_name_custom ?? null,
      ...
      project_display: project
        ? (project.project_name ?? project.customer_name ?? null)
        : (r.project_name_custom ?? null),
    };
  });
