...
| Marketing + Design follow-ups | 🔜 Next | Remaining UI polish (not blocking): (1) wire `ClosureApprovalsPanel` server component into the founder dashboard `/dashboard` page, (2) refactor `project-files.tsx` into a shared `entity-files.tsx` that accepts `entityType: 'lead' \| 'project'` + rebuild `/design/[leadId]` per-lead workspace with BOM editor + "Mark Design Confirmed" flow (D5 of the plan), (3) redesigned Quote tab with one-click "Generate Quick Quote" and "Finalize Detailed Proposal" buttons + inline BOM picker over price_book + consultant picker, (4) production deployment of migrations 051/052/053. The existing `/leads/[id]/proposal` tab still serves quotes unchanged in the interim. Plan file: `C:\Users\vivek\.claude\plans\peaceful-swimming-castle.md`. |
