...
*Last updated: April 11, 2026 — Documents tab fix. (1) Drag-and-drop fix: FileRow filename changed from `<button>` to `<span role="button">` — buttons intercept mousedown events preventing HTML5 drag from starting on the parent `<div draggable>`. Added `draggable={false}` on all interactive child elements (download/delete buttons, action wrapper). Added `pointer-events-none` on decorative icons (GripVertical, file type, file size) to pass through drag events. (2) Auto-populated documents: DocumentsTab server component now fetches DC data (getStepDeliveryData), QC inspections (getStepQcData), and survey data (getStepSurveyData) in parallel via Promise.all alongside existing handover pack query. New GeneratedDocRow component renders system-generated entries with blue icon, status badge, and PDF download link. Survey Report → Customer Documents box. Delivery Challans → DC box (one row per DC with item count, date, status, PDF link to /api/projects/[id]/dc/[dcId]). QC Reports → Documents/Approvals box (per inspection with inspector name, date, approval status, PDF link to /api/projects/[id]/qc/[inspectionId] for submitted/approved). Auto-generated entries appear above uploaded files separated by dashed border. 2 files changed, 0 type errors.*
