...
| Marketing + Design UI | 🔜 Next session | Lib layer is complete + compiling. Remaining work is purely UI construction: (1) new `/sales` route that replaces `/leads` in sidebar (lead-stage-nav extended for Path A/B collapse + closure_soon), (2) `/sales/[id]` lead detail with new Quote tab exposing `createBudgetaryQuoteAction` / `finalizeDetailedProposal` as one-click buttons, BOM picker component over price_book, consultant picker component, closure band badge with margin gauge, (3) `/design/[leadId]` per-lead workspace reusing project-files.tsx pattern (refactor to shared `entity-files.tsx`) + BOM editor + "Mark Design Confirmed" flow, (4) `/partners` list page + `/partners/[id]` detail with payout review, (5) closure approval dialog + founder approval panel on dashboard, (6) `/payments` Follow-ups tab surfacing `tasks` where `category IN ('payment_followup', 'payment_escalation')`, (7) wire `readOnly` prop on `step-liaison.tsx` for `project_manager` role, (8) `middleware.ts` 307 redirects `/leads/*` → `/sales/*` and `/proposals` → `/sales`. Plan file: `C:\Users\vivek\.claude\plans\peaceful-swimming-castle.md`. |
