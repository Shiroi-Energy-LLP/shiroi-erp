...
*Last updated: April 9, 2026 — Projects page bug fixes + sticky filter bars. (1) Pagination fix: SearchInput.tsx — debounce effect was resetting page=1 whenever URL changed (e.g. clicking pagination buttons); fix is an early return when `value === urlValue`. (2) Status change fix: migration 032 (`032_fix_payment_followup_trigger_proposal_status.sql`) replaces `p.status IN ('approved', 'accepted')` with `p.status = 'accepted'` in create_payment_followup_tasks — 'approved' is not in the proposal_status enum and was blocking transitions to in_progress/completed/waiting_net_metering. (3) Bulk actions: new `bulkUpdateField` server action in inline-edit-actions.ts + status-change dropdown wired into projects-table-wrapper.tsx. (4) Sticky filter Card on 12 list pages (projects, leads, proposals, contacts, companies, invoices, procurement, profitability, payments/receipts, tasks, vendors, inventory) via `sticky top-0 z-20 shadow-sm`. (5) Projects page header stripped (Eyebrow + h1 removed — topbar already shows "Projects"). tsc --noEmit: 0 errors. Next: employee testing week, prod deployment.*
