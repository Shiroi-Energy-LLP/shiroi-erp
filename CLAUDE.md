...
| Price Book CSV import | ✅ Complete | scripts/import-price-book-csv.ts: reads local CSV export (Google Sheet is private — founder exports first), minimal RFC 4180-ish parser, 22-category CATEGORY_MAP, rate parser strips ₹ + commas, batch inserts 100/row via Supabase service key, --dry-run flag. Usage: `npx tsx scripts/import-price-book-csv.ts path/to/export.csv`. Items with zero rate inserted with base_price=0, show "Rate pending" badge. **Awaiting Vivek to export Manivel's 217-item sheet and run the script.** |
