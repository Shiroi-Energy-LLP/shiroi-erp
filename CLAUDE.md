...
| AMC Module V2 | ✅ Complete | Complete rewrite of /om/amc page. 6 summary cards (Total Contracts with free/paid split, Active, This Week visits, Upcoming, Completed, Overdue with alert icon). Filter bar (contract status, visit status, project). Visits table: 9 columns (Project, Visit #, Type, Scheduled Date, Engineer, Status, Completed, Rescheduled count, Edit). Contracts table: 8 columns (Contract #, Project, Type with Free/Paid badge, Start, End, Annual Value, Visits, Status). VisitStatusToggle — inline dropdown (scheduled/confirmed/completed/rescheduled/cancelled/missed). RescheduleVisitDialog — edit date/engineer/reason. 4 new server actions (updateVisitStatus, rescheduleVisit, assignVisitEngineer, getAllAmcData). |
