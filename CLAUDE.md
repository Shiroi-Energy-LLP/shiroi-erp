...
*Last updated: April 9, 2026 — Projects screen overhaul per Manivel's spec: migration 031 collapses project_status 11→8, fixes FK error on inline status edit (log_project_status_change now looks up employee_id from profile_id), auto-creates Projects from accepted Proposals (trigger_proposal_accepted_create_project), rebuilds idx_projects_active with new enum literals. UI: remarks hidden by default, customer_name clickable, project numbers stripped of SHIROI/PROJ/ prefix via formatProjectNumber(). 14 code files updated across column-config, data-table, project-status-badge, pm-donut-chart, advance-status-button, project-status-helpers, profitability/page, payments/page, payments-overview-queries, amc-actions, dashboard-queries, pm-queries, project-step-actions, tasks-actions. Data: 314 existing projects correctly remapped (267 completed + 14 in_progress + 14 holding_shiroi + 14 waiting_net_metering + 5 order_received). tsc --noEmit: 0 errors. Next: employee testing week, prod deployment.*
