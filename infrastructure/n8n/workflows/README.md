...
**And: changing `settings.timezone` on an already-active cron workflow is NOT enough — n8n caches the cron schedule at activation time.** Deactivate → reactivate is required for the schedule to re-register with the new TZ. Caught on 2026-05-04 morning when crons fired in UTC despite the workflow's `settings.timezone: "Asia/Kolkata"`.

**Further (caught 2026-05-19): `settings.timezone` doesn't control the cron scheduler at all in n8n 2.x — only the trigger node's own `parameters.timezone` does.** Workflow-level `settings.timezone` controls DateTime expressions inside workflow logic (e.g., `DateTime.now().setZone(...)`) but NOT the scheduleTrigger node's cron evaluation. Even with workflow-level setting + container `GENERIC_TIMEZONE=Asia/Kolkata`, crons fired in UTC. The actual fix: add `parameters.timezone: "Asia/Kolkata"` (or equivalent) to every `n8n-nodes-base.scheduleTrigger` node. `scripts/fix-n8n-trigger-timezone.ts` does this for all workflows. After running, re-push AND deactivate→reactivate each cron workflow so n8n re-registers the schedule with the new TZ. Verified the hard way 2026-05-02: a wrong-Vinodh-number was fixed via sed in `.env` + `restart`, but the container kept the old value cached and kept sending to the wrong number until force-recreate. Inspect with `docker exec shiroi-automation-n8n-1 env | grep VINODH` to confirm.
